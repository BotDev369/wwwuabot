 ...
 *   • якщо рядок **почався** всередині `/* … *​/` і в ньому після `*​/` є код —
 *     це код (у рядку справді є інструкції).
 *
 * **Це оцінка, а не парсер.** Рядок зі `//` усередині рядкового літерала
 * (адреса `https://…`) — код, бо значущий вміст почався до `//`; навпаки,
 * `{/* … *​/}` у JSX рахується кодом, бо починається з `{`. Обидві похибки
...
/** Стан між рядками: `/* … *​/` не закінчується на межі рядка. */
