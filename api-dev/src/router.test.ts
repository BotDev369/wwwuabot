 ...
 * Межа, яку він тримає (AGENTS.md §5 і §7): авторизує **тільки** cookie
 * `admin_session`. Секрети в заголовках (`X-Admin-Secret` / `X-Bot-Token`),
 * `/db-proxy` і легасі вебхук-ендпоїнти не існують — вони авторизували б
 * адмін-дію в обхід сесії. Ці тести фіксують межу, щоб вона не протекла знову.
 *
...
describe("адмін-гейт", () => {
  ...
  it("⛔ НЕ приймає секрет у заголовку (AGENTS.md §5)", async () => {
...
// ── Легасі-поверхні, яких не існує (AGENTS.md §7) ────────────────
