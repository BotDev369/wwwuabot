...
export function ScenariosV2Table() {
  const { items, sortField, sortDir, setSort, filter, setFilter, groupBy, setGroupBy } =
    useScenariosStore();
            ...
            <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
              {FILTER_CHIPS.map((chip) => {
                ...
                return (
                  ...
                  >
                    {chip.icon && ico(chip.icon, 14)} {chip.label}{" "}
                    <span style={{ fontSize: 10, opacity: 0.8, marginLeft: 2 }}>{count}</span>
                  </button>
                );
              })}
            </div>
            ...
            <div style={{ display: "flex", gap: 4 }}>
              {GROUP_MODES.map((mode) => {
                ...
                return (
                  ...
                  >
                    {ico(mode.icon, 14)} <span className="scn-hide-mobile">{mode.label}</span>
                  </button>
                );
              })}
            </div>
