...
export function ScenarioRow({ scenario, isSelected, onSelect, onOpen }: ScenarioRowProps) {
  ...
  return (
    <tr className={`usr-row${isSelected ? " usr-row--selected" : ""}`} onClick={onSelect} style={{ cursor: "pointer" }}>
      ...
      <td>
        <span style={{ display: "inline-flex", alignItems: "center", gap: 4, padding: "2px 8px", fontSize: 11, fontWeight: 500, borderRadius: 10, background: badge.color, color: "#fff" }}>
          {ico(badge.icon, 12)} {badge.label}
        </span>
      </td>
      <td className="usr-td-date" title={scenario.updated_at}>{relativeTime(scenario.updated_at)}</td>
      <td>
        <button className="wb-btn wb-btn-secondary" onClick={(e) => { e.stopPropagation(); onOpen(); }}
          style={{ fontSize: 12, padding: "4px 10px" }} title="Відкрити сценарій"
        >{ico("link", 14)} Відкрити</button>
      </td>
    </tr>
  );
}
