...
function RichTextSection({
  ...
  if (!RichTextField)
    return <div style={{ padding: 12, color: "var(--text-muted)" }}>Завантаження редактора...</div>;
...
function RichDataEditor({ value, onChange }: { value: string; onChange: (v: string) => void }) {
    ...
    <>
      ...
      {mode === "visual" ? (
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {blocks.length === 0 ? (
            <div
              style={{ padding: 16, textAlign: "center", color: "var(--text-muted)", fontSize: 13 }}
            >
