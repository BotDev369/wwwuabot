  ...
  return (
    <div className="tg-message" style={{ maxWidth: 380, margin: "0 auto" }}>
      {richData.map((block, i) => (
        <RichBlock key={i} block={block as Record<string, unknown>} />
      ))}
    </div>
  );
}
...
function RichBlock({ block }: { block: Record<string, unknown> }) {
  ...
  if (type === "photo") {
    return (
      <img
        src={String(block.url || block.photo_url || "")}
        alt=""
        className="tg-photo"
        onError={(e) => {
    ...
    return (
      <div className="tg-blockquote">
        ...
        {children.map((child: unknown, i: number) => (
          <div key={i} style={{ marginTop: 4, fontSize: 13, color: "var(--text-muted)" }}>
  ...
  return <div className="tg-unknown">[{type || "unknown"}]</div>;
}
