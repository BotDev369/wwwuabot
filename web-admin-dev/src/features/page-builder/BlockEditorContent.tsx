...
interface BlockEditorContentProps {
  ...
  BlockEditorComponent: React.ComponentType<{
    block: PageBlock; zone: BlockZone; context: BlockContext; depth?: number;
    onUpdateProps: (blockId: string, props: Record<string, unknown>) => void;
...
export function BlockEditorContent({
  block, zone, context, definition, showConditions, availableTypes,
  onUpdateProps, onUpdateConditions, onChangeType,
  onAddChild, onRemoveChild, onUpdateChildProps, onUpdateChildName, onUpdateChildConditions,
  depth, BlockEditorComponent,
  ...
  return (
    <div style={{ padding: 10, borderTop: "1px solid var(--border)" }}>
      {definition?.description && (
        <div style={{ fontSize: 12, color: "var(--text-secondary)", marginBottom: 8, fontStyle: "italic" }}>{definition.description}</div>
      )}

      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8, flexWrap: "wrap" }}>
        <select value={block.type} onChange={(e) => onChangeType(block.id, e.target.value)}
          style={{ fontSize: 13, padding: "4px 8px", border: "1px solid var(--border)", borderRadius: 4, background: "var(--bg-1)" }}
        >
          {availableTypes.map((def) => <option key={def.type} value={def.type}>{def.label}</option>)}
        </select>
        <span style={{ fontSize: 11, color: "var(--text-secondary)" }}>id: {block.id.slice(0, 8)}</span>
      </div>

      {showConditions && (
        <ConditionsPanel conditions={block.conditions} onUpdate={(conditions) => onUpdateConditions(block.id, conditions)} />
      )}
      ...
      {definition?.type !== "divider" && (
        <div style={{ marginTop: 10 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6, flexWrap: "wrap", gap: 6 }}>
            <span style={{ fontSize: 12, fontWeight: 500 }}>Вкладені блоки ({block.children?.length ?? 0})</span>
            <button className="wb-btn wb-btn-secondary pb-be-add-btn" onClick={() => onAddChild(block.id, "text")}
              style={{ fontSize: 11, padding: "4px 10px", whiteSpace: "nowrap" }}>+ Додати</button>
          </div>
          {block.children && block.children.length > 0 && (
            <div style={{ marginLeft: 8 }}>
              {block.children.sort((a, b) => a.order - b.order).map((child) => (
                <BlockEditorComponent key={child.id} block={child} zone={zone} context={context} depth={depth + 1}
                  ...
                  onChangeType={(_, newType) => { onUpdateChildProps(block.id, child.id, { ...child.props, _newType: newType }); }}
