...
export function ArrayFieldEditor({ fieldKey, title, propSchema, currentValue, onChange }: ArrayFieldEditorProps) {
  const [mode, setMode] = useState<"form" | "json">("form");

  const itemSchema = (propSchema.items as Record<string, unknown>) || {};
  const isObjectItems = Boolean(
    itemSchema.type === "object" && itemSchema.properties && typeof itemSchema.properties === "object",
  );
  ...
  const handleDeleteItem = (index: number) => {
    onChange(fieldKey, itemsArray.filter((_, i) => i !== index));
  };
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          {isObjectItems && (
            <button type="button" onClick={() => setMode("form")}
              style={{ fontSize: 11, padding: "2px 6px", borderRadius: 4, border: "1px solid var(--border)", background: "var(--bg-2, #f3f4f6)", color: "var(--text-primary)", cursor: "pointer" }}
            >Форма</button>
          )}
        </div>
        <textarea
  ...
  return (
    <div key={fieldKey} style={{ display: "flex", gap: 8, flexDirection: "column", marginTop: 4 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <label style={{ fontSize: 12, fontWeight: 500, color: "var(--text-secondary)" }}>{title} ({itemsArray.length})</label>
        <button type="button" onClick={() => setMode("json")}
          style={{ fontSize: 11, padding: "2px 6px", borderRadius: 4, border: "1px solid var(--border)", background: "transparent", color: "var(--text-secondary)", cursor: "pointer" }}
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        {itemsArray.map((item, index) => {
          ...
          return (
            <div key={index} style={{ display: "flex", gap: 6, alignItems: "center", padding: "6px 8px", background: "var(--bg-2, rgba(128,128,128,0.05))", border: "1px solid var(--border, #e5e7eb)", borderRadius: 6, flexWrap: "wrap" }}>
              ...
              <button type="button" onClick={() => handleDeleteItem(index)}
                style={{ border: "none", background: "transparent", color: "var(--danger, #ef4444)", cursor: "pointer", padding: "2px 6px", fontSize: 14, lineHeight: 1 }}
                title="Видалити пункт"
              >✕</button>
            </div>
          );
        })}
        <button type="button" onClick={handleAddItem} className="wb-btn wb-btn-secondary"
