...
18. **Нотатки — один спільний екран, і його вигляд — чисті функції.** Список, смуга
    ...
    Самі вибори — **клітинки-знаки в одному ряду** (`.wb-note-tool`): та сама клітинка, що у
    вкладок композера — одне правило на всіх, без рамки й тла, 44px під палець. Ні підпису, ні
    ...
    «що вибрано і як це зняти» — правило, а не розмітка. Знаки різні навмисно (`sort` — порядок,
    `layers` — групи, `hash` — хештег): один знак на дві дії не каже нічого. Картка — **кнопка на всю ширину**
