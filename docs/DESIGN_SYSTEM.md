...
18. **Нотатки — один спільний екран, і його вигляд — чисті функції.** Список, смуга
    ...
    **кожен**: вибір звужує; не рядок-сентевел), а групи «за
    ...
    Смуга — **ОДИН ряд, який не переноситься** (`.wb-note-bar`): пошук ліворуч, три вибори —
    **клітинки-знаки** (`.wb-note-tool`: та сама клітинка, що у вкладок композера — одне правило
    ...
    ряду одна на всі контроли, `--note-row-h`) поруч праворуч. Поле пошуку в спокої завширшки
    ...
    **акордеон із двома рядками інфо, і всі закриті**: рядок 1 — початок тексту (він і
    ...
    голосні, як він, роблять зі списку суцільну сітку, де око ні за що не чіпляється.
    ...
    треба (правило 13). Проміжок між двома рядками інфо — найменший (`--sp-1`), бо це один
    підпис під одним, а не сусідні блоки: більший читається як порожнеча в картці.
