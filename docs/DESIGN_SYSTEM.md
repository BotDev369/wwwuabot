...
20. **«МоїКонтакти»: контакт — запис, а лінк — його поле; приєднання — два кроки.** Екран (`/contacts`) зводить спільний
    ...
    **знайдений** тег стоїть акцентом). **Дотик розгортає рядок**: у тілі — етапи (`.wb-contact-stage*`, «Запрошено» →
    ...
    немає**, бо id людини власник не знає, його бачить бот. **Числа — три пари «знак + цифра» (Всього / Бот / Платформа) у ряд із
    назвою екрана**, останнім у тому ж рядку — **коло з «+»** (`.wb-page-add`, спільна мірка каркаса: 36px, `--radius-full`, `padding: 0`; акцентне тло лишається **тільки** в нього — воно й робить його головною дією, коли підпису немає). Знаки різні навмисно (`users` / `bot` / `globe`): три однакові силуети читались би як одне число. Заливки й підписів у шапці **немає** — плашка з `--field-bg` важила б більше за саму цифру, а місце належить назві; слово лишилось у `dt` і сховане (`.wb-contact-stat-word`: `1px`, `clip-path: inset(50%)`, а **не** `display: none`), бо сам знак числа не називає, і число й знак стоять **акцентом**.
    ...
    **людей, а не картки** (два лінки на одну людину дають два записи й одного в лійці; той, хто на платформі,
    ...
    сама людина, що …» (`.wb-contact-twin`). Блок «схема залучених» **теж зник**, а «+» питає ім'я, створює запис, **одразу складає лінк, кладе його в буфер** і відкриває форму. Заголовок екрана — **«Контакти»** (без «Мої…»: у шапці він читається разом зі знаками, і довге слово відсувало б числа), а **«+» стоїть і в «Нотатках»** — та сама мірка й те саме правило: у шапці місце дії, а не підпису, і назву кнопки несе `aria-label`.
...
| `packages/shared/src/styles/contacts.css` | Кирпичики «Контактів» (`.wb-contact*`): три знаки з числами (у шапці), рядок-акордеон (і плитковий режим), тіло з етапами й лінком, форма · `packages/ui/src/contacts/` — `ContactList` (групи + акордеон), `ContactSheet` (форма), `ContactTotals`, `ContactsToolbar` і чисті `view.ts` / `scheme.ts` |
