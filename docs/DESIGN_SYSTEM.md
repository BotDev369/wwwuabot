...
18. **Нотатки — один спільний екран, і його вигляд — чисті функції.** Список, смуга
    ...
    **кожен**: вибір звужує; не рядок-сентевел), а групи «за
    ...
    Смуга — **ОДИН ряд** (`.wb-note-bar`): пошук ліворуч, три вибори — **клітинки-знаки**
    (`.wb-note-tool`: та сама клітинка, що у вкладок композера — одне правило на всіх, без
    рамки й тла, 44px під палець) поруч праворуч. Поле пошуку в спокої завширшки зі свій
    підпис («Пошук») і розкривається (`--open`) на фокус або запит: порожнє поле на всю
    ширину забирало місце саме в тих трьох, за якими людина приходить. Ні підпису, ні
    заливки, ні **вибраного** в клітинці немає: «що зараз вибрано» показує чип поруч
    (`.wb-note-chip` — кнопка, тому дотик прибирає вибір), а ім'я клітинки й поточний стан
    читає `aria-label`. Типовий вибір чипа **не має** — постійні «Змінені» й «За днями» були б
