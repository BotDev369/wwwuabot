...
18. **Нотатки — один спільний екран, і його вигляд — чисті функції.** Список, смуга
    ...
    **кожен**: вибір звужує; не рядок-сентевел), а групи «за
    ...
    Смуга — **ОДИН ряд, який не переноситься** (`.wb-note-bar`): пошук ліворуч, чотири —
    **клітинки-знаки** (`.wb-note-tool`: та сама клітинка, що у вкладок композера — одне правило
    на всіх, без рамки й тла, але **32px**, а не планка пальця 44px: у смузі чотири контроли в
    ряд, і високий ряд забирав у списку більше екрана, ніж сам список, до якого веде — мірка
    ряду одна на всі контроли, `--note-row-h`) поруч праворуч. Три з них — **вибори**, а
    четвертий (`.wb-note-tool--toggle`) — **перемикач** «розгорнути / згорнути всі нотатки»:
    ...
    **окремим рядом під смугою** (`.wb-note-chips`): вони не клітинки керування, а те, що ці
    клітинки змінили. Поле пошуку — **один контрол, і контрол тут саме поле**, а не оболонка з
    іконкою (`.wb-note-search` — лише ряд). У спокої навколо поля немає ні заливки, ні рамки:
    воно стоїть поруч із трьома клітинками, і залите поле читалось як ще одна кнопка. На дотик
    заливка з **розмитим контуром** (`--note-search-glow` — два розмиті ореоли, без лінії)
    з'являється навколо **місця, де пишуть**, а не навколо іконки. Мірки поля тут свої
    (`--note-row-h`), бо мірки поля-в-полі зробили б ряд учетверо вищим за клітинки поруч; тому
    ...
    **свій** (`.wb-note-search-clear`), а не нативний: нативний у WebView малюється окремою
    ...
    **вибраного** в клітинці немає: «що зараз вибрано» показує чип
    (`.wb-note-chip` — кнопка, тому дотик прибирає вибір), а ім'я клітинки й поточний стан
    читає `aria-label`. Стан смуги стереже `NotesToolbar.test.tsx` — він читає й CSS, тож
    перенос, заливка в спокої або мірка ряду понад 32px провалюють тест, а не тихо
    повертаються. Типовий вибір чипа **не має** — постійні «Змінені» й «За днями» були б
    тими самими овалами, від яких ми тікали. Склад чипів — чиста функція `viewChips(view)`, бо
...
20. **«МоїКонтакти»: контакт — запис, а лінк — його поле.** Екран (`/contacts`) зводить спільний
    `@wwwuabot/ui/contacts` (кирпичики `.wb-contact*` у власному `contacts.css` — як
    `theme-panel.css`, бо це цілісна деталь). **Список** — той самий спільний кирпичик вигляду
    (`.wb-collection*` + `CollectionViewSwitch`: рядки / картки 1 / картки 2), і в рядку
    **немає жодної дії**: там лише номер, ім'я, хто це (`@хендл`, `id`) і стан **словом**
    («без лінка» / «лінк чекає» / «приєднався»), а хештеги — підписами (`.wb-contact-tag`,
    не чип: бренди задають чипу мірки з `!important`). **Дотик по контакту відкриває картку**
    (`ContactSheet`, та сама поверхня `.wb-sheet`), і там живуть **усі** поля: ім'я, `@username`,
    Telegram-id, хештеги, примітки й особистий лінк. Форма одна на створення й правку, а
    **дії віддаються нагору** (`onSave` / `onDelete` / `onMakeLink` / `onCopyLink`): буфер,
    діалоги й підтвердження належать оболонці (та сама межа, що в нотатках). Лінк показується
    **за станом контакту**: немає коду → «Створити лінк», код є → сам діплінк і «Копіювати»,
    приєднався → **ані того, ані того** (закріплення стається раз, тож посилання нічого не
    закріпило б — та сама чесність, що в заглушок меню). **«Схема залучених»** — три числа
    (контактів / запрошено / приєднались) плюс гілка, у якій видно, хто з контактів залучив
    далі; другий рівень показується **лише коли він є** («залучив(ла) ще 0» — рядок заради
    нуля). Схема малюється з **тих самих контактів**, а не окремим запитом, і числа рахує
    чиста `contactStats` — тут помилка виглядає як неправильна цифра, а не як зламаний код;
    а «контакт без лінка» в «очікують» **не рахується**: чекати нічого, бо нічого не надіслано.
    Дія екрана одна — **«Додати контакт»**: питає ім'я, створює запис, **одразу складає лінк і
    кладе його в буфер** та відкриває картку. Якщо ж ім'я бота ще невідоме, контакт лишається
    без лінка: «створити» й «запросити» — дві дії, і друга стається тоді, коли її треба.
...
| `packages/ui/src/notes/` | `NotesList` (картка-акордеон) і `NotesToolbar` + чисті `view.ts` — список нотаток: пошук, фільтр, сортування, групування (екран платформи `/notes`) |
