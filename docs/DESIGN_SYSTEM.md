...
20. **«МоїКонтакти»: контакт — запис, а лінк — його поле; приєднання — два кроки.** Екран (`/contacts`) зводить спільний
    ...
    цілісна деталь), а список улаштований **як «МоїНотатки»**: групи, акордеони й та сама **спільна смуга
    керування** з `@wwwuabot/ui/collection` (`CollectionToolbar`, чипи — `buildViewChips`, правила хештега й днів —
    `tags.ts` / `days.ts`). Контакти підставляють лише свої варіанти й свої слова: друга копія смуги розійшлася б
    із першою на першій же правці. Закритий рядок показує **два рядки інфо**: номер, ім'я з датою-часом зміни і стан
    **словом** («без лінка» / «лінк чекає» / «зайшов у бота» / «приєднався»), а хештеги — підписами
    (`.wb-contact-tag`, не чип: бренди задають чипу мірки з `!important`; **знайдений** тег стоїть акцентом).
    **Дотик розгортає рядок**: у тілі — етапи (`.wb-contact-stage*`, «Запрошено» → «Зайшов у бота» → «Зайшов на
    платформу», і видно **всі три**: різниця між другим і третім і є те, заради чого екран існує), лінк **за станом**
    (немає коду → «Створити лінк», код є → діплінк і «Копіювати», за лінком уже прийшли → **ані того, ані того**),
    обидві дати, глибина гілки («Залучив(ла) ще N») і дії — а **дії віддаються нагору** (`onEdit` / `onDelete` /
    `onMakeLink` / `onCopyLink`): буфер, діалоги й підтвердження належать оболонці (та сама межа, що в нотатках).
    **Правка — у формі** (`ContactSheet`, та сама поверхня `.wb-sheet`), і там **лише власні поля** (ім'я,
    `@username`, хештеги, примітки): факти показує тіло рядка, бо показане двічі тримають узгодженим у двох місцях.
    **Поля «Telegram ID» немає** — id людини власник не знає, його бачить бот. **Числа лійки («Запрошено / У боті /
    На платформі») стоять у шапці, поруч із назвою**, а не блоком під списком: за ними приходять саме тоді, коли
    список довгий; рахують вони **людей, а не картки** (два лінки на одну людину — два записи й одна людина; той,
    хто на платформі, порахований і в «у боті»), дубль підписаний «та сама людина, що …» (`.wb-contact-twin`), а під
    числами стоїть пояснення (`.wb-contact-twins-note`). Окремий блок «схема залучених» **зник** — числа переїхали в
    шапку, гілка в тіло рядка, а дія одна: **«Додати контакт»** питає ім'я, створює запис, **одразу складає лінк,
    кладе його в буфер** і відкриває форму.
...
| `packages/shared/src/styles/contacts.css` | Кирпичики «МоїКонтакти» (`.wb-contact*`): числа лійки (у шапці), рядок-акордеон (і плитковий режим), тіло з етапами й лінком, форма · `packages/ui/src/contacts/` — `ContactList` (групи + акордеон), `ContactSheet` (форма), `ContactTotals`, `ContactsToolbar` і чисті `view.ts` / `scheme.ts` |
