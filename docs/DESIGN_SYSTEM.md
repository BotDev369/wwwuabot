...
18. **Нотатки — один спільний екран, і його вигляд — чисті функції.** Список, смуга
    ...
    **клітинки-знаки** (`.wb-note-tool`: та сама клітинка, що у вкладок композера — одне правило
    на всіх, без рамки й тла, 44px під палець) поруч праворуч. Поле пошуку в спокої завширшки
    ...
    **окремим рядом під смугою** (`.wb-note-chips`): вони не клітинки керування, а те, що ці
    клітинки змінили. Поле пошуку — **один контрол, і контрол тут оболонка**
    (`.wb-note-search`): заливку, мірки й кільце фокуса (`:focus-within`) несе вона, бо іконка
    живе в ній, а внутрішній `.wb-input` не несе нічого свого (мірки, заливка й кільце
    обнулені, `!important` — бо бренди задають ті ж властивості полю з `!important`, див.
    правило 13). Без цього в розкритому полі видно «поле в полі». ✕ у полі — **свій**
    (`.wb-note-search-clear`), а не нативний: нативний у WebView малюється окремою коробкою зі
    своїм тлом і не піддається стилю; бренди дають оболонці пошуку ті самі мірки й те саме
    кільце, що й будь-якому полю (`apple.css` / `android.css`). Ні підпису, ні заливки, ні
    **вибраного** в клітинці немає: «що зараз вибрано» показує чип
    ...
    перенос або друге кільце всередині поля провалюють тест, а не тихо повертаються. Типовий вибір чипа **не має** — постійні «Змінені» й «За днями» були б
