...
20. **«МоїКонтакти»: контакт — запис, а лінк — його поле; приєднання — два кроки.** Екран (`/contacts`) зводить спільний
    ...
    **немає жодної дії**: там лише номер, ім'я, хто це (`@хендл`, `id`) і стан **словом**
    («без лінка» / «лінк чекає» / «зайшов у бота» / «приєднався»), а хештеги — підписами (`.wb-contact-tag`,
    не чип: бренди задають чипу мірки з `!important`). **Дотик по контакту відкриває картку**
    (`ContactSheet`, та сама поверхня `.wb-sheet`), і там живуть **усі власні** поля: ім'я,
    `@username`, хештеги, примітки й особистий лінк. Форма одна на створення й правку, а
    **дії віддаються нагору** (`onSave` / `onDelete` / `onMakeLink` / `onCopyLink`): буфер,
    діалоги й підтвердження належать оболонці (та сама межа, що в нотатках). **Поля «Telegram
    ID» у картці немає** — id людини власник не знає, його бачить бот; замість поля — список
    **етапів** (`.wb-contact-stage*`, підписи — сталим стовпчиком): «Запрошено» → «Зайшов у
    бота» → «Зайшов на платформу», і видно **всі три**: різниця між другим і третім і є те,
    заради чого екран існує. Лінк показується **за станом**: немає коду → «Створити лінк»,
    код є → сам діплінк і «Копіювати», за лінком уже прийшли → **ані того, ані того**.
