...
20. **«МоїКонтакти»: контакт не заводять — його запрошують лінком.** Екран (`/contacts`) зводить
    ...
    Другий рівень показується **лише коли він є**: «залучив(ла) ще 0» — рядок заради нуля.
    Схема малюється з **тих самих лінків**, а не окремим запитом: це той самий факт під іншим
    ...
    Картка віддає дії нагору (`onCopy` / `onRename` / `onDelete`): буфер обміну, питання імені
    й підтвердження належать оболонці, а не картці (та сама межа, що в нотатках). Посилання живе **в самій картці** —
    ...
    лінк = один контакт**.
    І працюють із ним **усіма чотирма діями**: створити, побачити, **перейменувати** й прибрати — правка
    чіпає **підпис власника**, а не ім'я людини (те приходить із профілю, `contactDisplayName`), стоїть біля
    імені, а не в ряду `Копіювати / Прибрати` (третя кнопка там на телефоні розривається), і без змін на сервер не йде.
