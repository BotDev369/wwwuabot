...
| `scenarios` | `api-dev` | api-dev (`ensureBase`, портал) | **bot-dev читає**; api-dev редагує (`/api/portal/scenarios/*`) | контент бота й порталу |
| `scenarios-admin` | `api-dev` | api-dev (фабрика, адмінка) | api-dev (`/api/admin/scenarios/*`) | окремий контент-набір адмінки (вкладка «Адмін») |
...
## Відоме: дві таблиці сценаріїв (рішення не ухвалено)

`scenarios` і `scenarios-admin` мають **однакову** схему (це забезпечує реєстр), але лишаються
двома таблицями: у адмінці це дві вкладки — «Портал» (те, що бачить бот) і «Адмін» (окремий
набір). Об'єднання в одну таблицю з колонкою `scope` — **відкрите рішення власника**
([`CONSOLIDATION_PLAN.md`](./CONSOLIDATION_PLAN.md) §4, пункт 1).
