...
| `pages` | `api-dev` | api-dev, `ensureTables(db, ["pages"])` | **поки що ніхто** — читачі переїжджають у фазі 3 ([`CONTENT_MODEL.md`](./CONTENT_MODEL.md) §4) | **єдине сховище контенту:** рядок = сторінка вебу (`blocks`) + подання в боті (`bot`); `kind = 'collection'` — група сторінок |
...
чіпаються: міграція **тільки додає**, а рядок, який не пройшов перевірку (чужа адреса чи чужий
`codeword`), лишається в старій таблиці й потрапляє у звіт — рішення ухвалює власник.
