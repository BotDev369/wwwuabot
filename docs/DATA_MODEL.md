...
**Стирання — у обох, і це не дрібниця, а наслідок схеми.** Рядок переписки один на пару, тож «очистити»
(`POST /api/messages/clear`) стирає повідомлення **разом із `last_message_*`**: останок у списку є копією
останнього повідомлення, і без цього список показував би текст, якого вже немає. Розмова лишається —
у ній можна писати далі, — і `greeted_at` теж: вітання одноразове, а повторювати його після чистки
означало б писати в розмову те, що людина щойно стерла. «Видалити» (`POST /api/messages/delete`)
прибирає і сам рядок — разом із `greeted_at`, тож наступне відкриття починається з нуля. Обидві дії
вимагають **того самого зв'язку**, що читання, і перевіряють його першими (§7): інакше чужого `peer`
вони б витирали чужу історію, а код відповіді (404 проти 400) підказував би, що вона існує.
