...
**Чернетка тримає рядок у списку — єдиний виняток із приховування.** `listConversations` читає їх разом
із розмовами й причіплює до рядка: без цього ненадісланий текст не видно **ніде** (прибрана розмова зі
списку зникає, а про чернетку знає лише форма).
