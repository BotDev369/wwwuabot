...
export function CompareSetupPage({ onScenarioName }: { onScenarioName: (name: string | null) => void }) {
            ...
            {dates.map((d, i) => (
              <li key={`${d}-${i}`} className="date-item">
                ...
                <span className="date-item-actions">
                  <button className="icon-btn" onClick={() => move(i, -1)} disabled={i === 0} aria-label="Вгору">
                    ↑
                  </button>
                  <button className="icon-btn" onClick={() => move(i, 1)} disabled={i === dates.length - 1} aria-label="Вниз">
                    ↓
                  </button>
                  <button className="icon-btn danger" onClick={() => remove(i)} aria-label="Видалити">
                    ✕
                  </button>
                </span>
              </li>
