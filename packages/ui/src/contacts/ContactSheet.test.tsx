 ...
 *
 * Перевіряємо те, що вирішує, чи працює екран: поля є **всі** (номер і дати,
 * ім'я, `@username`, Telegram-id, хештеги, примітки), порожнє ім'я зберегти не
 * дає, а лінк показується **за станом контакту** — немає коду → кнопка
 * «Створити лінк»; код є → сам діплінк і «Копіювати».
 *
 * Окремо — найчесніше правило картки: **приєднаному лінк не пропонується**.
 * Закріплення стається лише раз, тож посилання, яке вже нікого не закріпить,
 * було б обіцянкою, якої воно не виконає.
 *
...
describe("ContactSheet", () => {
  it("тримає всі поля контакту й номер із датами одним рядком", () => {
    const html = render();
  ...
  it("показує поточні значення полів, а не порожні", () => {
    const html = render({
      username: "karas",
      telegramUserId: 6281898553,
      notes: "знайомий зі школи",
    });

    expect(html).toContain('value="karas"');
    expect(html).toContain('value="6281898553"');
  ...
  it("⛔ приєднаному контакту лінк не пропонується: він більше нікого не закріпить", () => {
    const html = render({
      code: "inv-8f3k2q",
      deepLink: "https://t.me/botdev_test_001_bot?start=inv-8f3k2q",
      telegramUserId: 6281898553,
