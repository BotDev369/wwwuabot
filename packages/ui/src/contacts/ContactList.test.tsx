 ...
 *
 * Перевіряємо те, що легко зламати мовчки: номер контакту порядковий і збігається
 * з місцем у списку, стан названий **словом** (а не самим кольором), а дій у
 * рядку немає — вони живуть у картці, яку рядок відкриває. Хештег при цьому
 * **підпис, а не чип**: у чипа бренди задають свої мірки з `!important`, і рядок
 * тегів виходив би вдвічі вищим за рядок із текстом.
 *
 ...
 *
 * Саме **в селекторі**, а не «одразу перед дужкою»: плитковий режим пише свої
 * правила тим самим класом у складі довшого селектора
 * (`.wb-collection--cards .wb-contact-card`), і пошук за сусідством дужки
 * повернув би тіло, що стоїть **перед** ним. Селектор звіряємо **цілим**: так
 * `rule(".wb-contact-card")` віддає базове правило, а плитковий варіант
 * знаходять окремим, повним селектором.
 */
...
describe("ContactList", () => {
  ...
  it("складений, але не відкритий лінк каже «лінк чекає»", () => {
    expect(
      render([contact(1, { code: "inv-8f3k2q", deepLink: "https://t.me/bot?start=x" })]),
    ).toContain("лінк чекає");
  });

  it("приєднаний контакт показує, хто це, і коли приєднався", () => {
    const html = render([
      contact(1, {
        username: "karas",
        telegramUserId: 555,
        joinedAt: "2026-09-18 03:40:00",
      }),
    ]);
    ...
    expect(html).not.toContain("без лінка");
  });
