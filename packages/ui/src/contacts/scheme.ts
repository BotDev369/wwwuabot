 ...
 * «Всього» ж рахує **усі записи** — так лійка читається без пояснень (плашки
 * йдуть за спаданням: всього → бот → платформа).
 *
...
export interface ContactStats {
  ...
  nested: number;
  /**
   * Скільки записів описують людину, про яку вже є запис вище — `total` мінус
   * люди.
   *
   * На екрані це вже не рядок-пояснення (власник бачить близнюків у списку,
   * де другий запис підписаний), але число лишається: саме воно показує, що
   * «Всього» більше за «Бот» не через втрату даних, а через два лінки на одну
   * людину.
   */
  duplicates: number;
}
