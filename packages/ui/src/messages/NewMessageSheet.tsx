 ...
 *
 * **Чернетку можна прибрати окремою дією.** Стерти текст і зберегти — теж
 * прибирає її (адресат не тримає рядка, див. `MessageDraftInput`), але це не
 * спосіб, а наслідок: прибрати ненадісланий лист мусить бути видно й названо,
 * тож у наявної чернетки стоїть третя дія — «Видалити». Вона є **тільки** в
 * наявної: новому листу нема чого прибирати, і кнопка, що нічого не робить, —
 * це кнопка, яку тицяють даремно.
 *
...
export function NewMessageSheet({
  ...
  /** Прибрати чернетку — за номером, не за вмістом полів: тіло листа тут ні до
      чого, адже чернетка ще нікуди не пішла. */
  async function remove(): Promise<void> {
    if (draft === null || busy) return;
    setBusy(true);
    const ok = await onDeleteDraft(draft.id);
    setBusy(false);
    if (ok) onClose();
  }
        ...
        content={
          <div className="wb-compose">
            ...
            <div className="wb-sheet-actions">
              ...
              {/* «Видалити» — не третій рівноправний вибір, а прибирання самого
                  листа, тож стоїть **останнім**: пара «зберегти / надіслати»
                  лишається там, де людина її знає, а знак кошика — під нею.
                  Питає підтвердження оболонка, а не форма (як `onClear`). */}
              {draft !== null && (
                <button
                  type="button"
