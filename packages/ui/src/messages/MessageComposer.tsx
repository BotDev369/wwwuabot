 ...
 * поле, яке їде разом із ним, зникає саме тоді, коли його шукають. Кирпичик
 * смуги тут свій — `.wb-thread-bar` (він же несе відступ під безпечну зону
 * знизу), а не спільний: смуга профілю мала два вузькі контроли й притискала
 * їх праворуч, а тут ширину ділить поле.
 *
...
export function MessageComposer({ sending = false, onSend }: MessageComposerProps): ReactElement {
  ...
  return (
    <form className="wb-thread-bar" onSubmit={(event) => void submit(event)}>
      <input
        type="text"
