          ...
          <div className="wb-sheet-bar">
            ...
            {closeAtBottom && (
              <button
                ...
                // 20px), а вихід — одна пілюля в КОЖНОМУ бренді. Своя форма —
                // це свій кирпичик, а не `!important` навперебій брендовому.
                className="wb-sheet-bar-close"
                onClick={onClose}
              >
                {/* Підпис словом, а не знаком `✕`: у цій смузі знаків багато
                    (перемикач вигляду), і два невідомі знаки поряд не кажуть,
                    який із них закриває поверхню. Ім'я для скрінрідера — той
                    самий підпис, тож окремий `aria-label` тут зайвий. */}
                Закрити
              </button>
            )}
          </div>
