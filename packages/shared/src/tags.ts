/**
 * Хештеги — чисті правила, без React і без стану.
 *
 ...
 * нормалізація вирішує, чи стане введене тегом — і чи знайдеться запис потім.
 * Правила, які вирішують долю даних, мусять бути в одному місці: та сама
 * функція вживається і в браузері (чипи в композері й у картці контакту), і на
 * сервері (що саме ляже в колонку `tags`). Дві копії тут розійшлись би тихо — і
 * «#Київ» з одного боку не знайшовся б «#київ» з другого.
 *
 * **Спільне на два домени.** Мітки є і в нотатки (`notes.tags`), і в контакту
 * (`contacts.tags`), і це один і той самий хештег: тому модуль лежить на верхньому
 * рівні, а не всередині `notes` — інакше картка контакту тягла б правила з чужого
 * домену, а другий набір правил розійшовся б із першим. Імена лишились з нотаток
 * (`MAX_NOTE_TAGS`) — перейменування нічого не додало б, крім руху в чотирьох
 * файлах.
 *
