...
/**
 * Ключі payload, які **шапка акаунта вже показала**: ім'я, прізвище, хендл і
 * фото. Список полів їх пропускає — інакше ім'я стояло б у розділі «Телеграм»
 * двічі поспіль, і людина шукала б між ними різницю.
 *
 * Живе тут, бо саме ці чотири ключі читають хелпери вище (`telegramName`,
 * `telegramHandle`, `telegramPhoto`) — один список на читання й на показ.
 */
export const TELEGRAM_HEAD_KEYS = ["first_name", "last_name", "username", "photo_url"] as const;
