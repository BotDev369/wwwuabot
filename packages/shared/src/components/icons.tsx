/**
 * ═══════════════════════════════════════════════════════════════════════════
 * ЗНАКИ ПРОДУКТУ — один набір на обидві оболонки
 * ═══════════════════════════════════════════════════════════════════════════
 *
 * Малюємо їх самі: тут не імпортується жодна бібліотека іконок. Кожен знак —
 * власний SVG-шлях у сітці 24×24, контуром (`stroke: currentColor`), тож він
 * бере колір тексту й працює в обох темах однаково.
 *
 * **Правило 26 — один знак = одне значення.** Два різні пункти з однаковою
 * картинкою змушують читати підпис, щоб їх розрізнити, — і саме так набір
 * перетворюється на «аби щось було». Тому знак не позичають у сусіднього
 * розділу: під нову суть малюють новий гліф.
 *
 * **Залитий близнюк** (`home-solid`) має сенс лише там, де пункт стоїть у
 * нижньому футері: активний слот фарбується тим самим знаком (правило 25).
 * Для решти «вибрано» показують колір і товщий штрих, тож близнюка не заводять —
 * близнюк без пункту футера це мертва картинка.
 *
 * Єдине джерело списку знаків — тип `IconName` нижче; у документації переліку
 * немає навмисно (він дрейфує). Рендер: `{icons[name]}` або `<Icon name />`.
 *
 * @module packages/shared/src/components/icons
 */
...
// Перелік живе в `./icon-names` (звичайний `.ts`, без JSX): його читають і
// воркери, які компілюються без `--jsx`. Тут лишається рівно те, чого там бути
// не може, — гліфи.
...
export const icons: Record<IconName, ReactElement> = {
  ...
  /** Сценарій — ланцюг кроків, а не «чотири квадрати»: рядок вузлів, з'єднаних
      стрілкою, каже, що тут по кроках ведуть. Раніше цей знак був піксель у
      піксель тим самим, що `grid` і `blocks`, тож «Сценарії», «2048» і
      «Створення» виглядали однаково (правило 26). */
  scenarios: icon([
    r(7.5, 2.5, 9, 6, 1.5),
    r(7.5, 15.5, 9, 6, 1.5),
    l(12, 8.5, 12, 12),
    pl("10.3 10.3 12 12 13.7 10.3"),
  ]),
  ...
  /** Панель зі стрілкою — сама дія «згорнути/розгорнути панель». Окремий гліф
      від `layout`: той тепер означає колонки, і панель із колонками більше не
      виглядають однаково. */
  "sidebar-toggle": icon([r(3, 4, 18, 16, 2), l(9.5, 4, 9.5, 20), pl("16.5 9.5 14 12 16.5 14.5")]),

  eye: icon([p("M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"), c(12, 12, 3)]),

  /* Пара до `eye` — «сховати». Один гліф на «показати» й «прибрати» змушував би
     читати підпис, щоб не натиснути не те: це дві протилежні дії з одним
     наслідком — видно / не видно (§4: знак без слова доречний лише там, де він
     єдиний у своєму ряду). */
  "eye-off": icon([
  ...
  /** Блоки-цеглини, а не «чотири квадрати»: так знак каже «з цього збирають».
      Раніше це був той самий гліф, що `grid` і `scenarios`. */
  blocks: icon([r(2.5, 14, 8.5, 7, 1.5), r(13, 14, 8.5, 7, 1.5), r(7.75, 3, 8.5, 7, 1.5)]),
  ...
  /** Нулик — знак гри «Хрестики-нулики». Названий за тим, чим є, а не за
      формою («circle» нічого не казала про вжиток). */
  "mark-o": icon([c(12, 12, 6)]),

  // Три предмети «каменя, ножиць, паперу» — самі речі, а не руки: руку в SVG
  // довелось би малювати десятками кривих, а предмет упізнається з першого
  // погляду й малюється двома лініями.
  /** Камінь — камінь, а не кільце: раніше це був самотній круг, який у грі
      читався як «порожньо», бо круг у нас означає ще й нулик. */
  rock: icon([
    p("M12 4.5c4.4 0 8 3.6 8 8s-3.6 7.5-8 7.5-8-3.1-8-7.5 3.6-8 8-8z"),
    p("M9 9.7l3-2.1 3 2.1"),
  ]),
  scissors: icon([p("M7.5 4L15 15.5"), p("M16.5 4L9 15.5"), c(7, 19, 2), c(17, 19, 2)]),
  paper: icon([r(6, 3, 12, 18, 2), l(9, 8, 15, 8), l(9, 12, 15, 12), l(9, 16, 13, 16)]),
  ...
  /** Дві колонки — знак блоку «Колонки» і вибору вигляду списку. */
  layout: icon([r(2.5, 3.5, 8.5, 17, 2), r(13, 3.5, 8.5, 17, 2)]),

  card: icon([r(2, 3, 20, 14, 2), p("M2 7h20"), p("M2 11h20")]),

  /** Таби — дві вкладки НАД панеллю, а не абстрактна рамка. Старий шлях виходив
      за сітку (координата 26 із 24), тож права вкладка обрізалась навпіл. */
  tabs: icon([
    r(2.5, 7, 19, 13.5, 2),
    p("M6.5 7V5.5A1.5 1.5 0 0 1 8 4h3a1.5 1.5 0 0 1 1.5 1.5V7"),
    p("M13.5 7V5.5A1.5 1.5 0 0 1 15 4h1.5A1.5 1.5 0 0 1 18 5.5V7"),
  ]),
  ...
  /** Хрестик — знак гри «Хрестики-нулики», не «закрити»: вікна закриває
      `close`, і це більше не той самий гліф. */
  "mark-x": icon([p("M7 7l10 10"), p("M17 7l-10 10")]),
  ...
  // --- Знаки блоків Page Builder ------------------------------------------
  //  Кожен блок має СВІЙ знак: пікер блоку — це список, а не меню, і два
  //  блоки з однією картинкою в ньому не відрізнити без читання підпису.

  /** Таблиця — шапка й колонки: знак блоку «Таблиця». */
  table: icon([r(3, 4, 18, 16, 2), l(3, 9.5, 21, 9.5), l(9.5, 9.5, 9.5, 20), l(15, 9.5, 15, 20)]),

  /** Прогрес — доріжка із заповненою частиною: знак блоку «Прогрес». */
  progress: icon([r(2.5, 9, 19, 6, 3), l(6, 12, 11, 12)]),

  /** Вибір — поле зі стрілкою всередині: знак блоку «Вибір» (select).
      Раніше він був `filter` — лійка обіцяла фільтр, а не випадаючий список. */
  select: icon([r(3, 5.5, 18, 13, 2), pl("14.5 10.5 17 13 19.5 10.5")]),

  /** Поле вводу — рамка й курсор. */
  field: icon([r(3, 5.5, 18, 13, 2), l(7, 9, 7, 15)]),

  /** Текстове поле — рамка з рядками тексту. */
  textarea: icon([r(3, 5.5, 18, 13, 2), l(6.5, 9.2, 17.5, 9.2), l(6.5, 14.8, 13.5, 14.8)]),

  /** Список із позначками — знак «оберіть із цього» (блоки «Вибір систем»
      і «Фіча»). Раніше один був `eye` (дивитись), а другий `sparkles`. */
  checklist: icon([
    pl("3.5 7 5.2 8.7 7.8 5.6"),
    l(10.5, 7, 20.5, 7),
    pl("3.5 13.4 5.2 15.1 7.8 12"),
    l(10.5, 13.4, 20.5, 13.4),
    l(10.5, 19, 17.5, 19),
  ]),

  /** Бейдж — пігулка з крапкою: сам знак бейджа, а не цінник `tag`. */
  badge: icon([r(2.5, 8, 19, 8, 4), c(7, 12, 1.1), l(10.5, 12, 18.3, 12)]),

  /** Кнопки — два ряди з підписами: знак блоку «Кнопки». Цього імені в наборі
      не існувало, тож картка блоку в пікері стояла взагалі без знака. */
  buttons: icon([r(2.5, 4, 19, 6, 2), l(6, 7, 9.5, 7), r(2.5, 14, 19, 6, 2), l(6, 17, 9.5, 17)]),

  /** Акордеон — дві складані секції: верхня розгорнута, нижня згорнута. */
  accordion: icon([
    r(2.5, 3.5, 19, 6.5, 2),
    l(6, 6.7, 12, 6.7),
    pl("16 7.2 18 5.2 20 7.2"),
    r(2.5, 14, 19, 6.5, 2),
    l(6, 17.2, 12, 17.2),
    pl("16 16.8 18 18.8 20 16.8"),
  ]),

  /** Відступ — проміжок між двома лініями: знак блоку «Відступ». Раніше це був
      `construction` (будівельний кран), який нічого не казав про порожнє місце. */
  spacer: icon([
    l(3, 4, 21, 4),
    l(3, 20, 21, 20),
    l(12, 8, 12, 16),
    pl("9.6 10.4 12 8 14.4 10.4"),
    pl("9.6 13.6 12 16 14.4 13.6"),
  ]),

  /** Hero — банер із заголовком, рядком і кнопкою: знак блоку «Hero». Раніше
      це були `sparkles`, які в продукті означають «згенеровано/нове». */
  hero: icon([r(2.5, 4, 19, 16, 2), l(6, 9, 16, 9), l(6, 12.5, 12, 12.5), r(6, 15, 6, 2.6, 1.3)]),
  ...
  /** Залитий близнюк нового `scenarios` — ті самі два вузли, лише фарбою:
      близнюк мусить бути ТИМ САМИМ знаком, інакше перехід пункту смикає
      картинку (а стара версія була ще й копією `grid`). */
  "scenarios-solid": solid([
    r(7.5, 2.5, 9, 6, 1.5),
    r(7.5, 15.5, 9, 6, 1.5),
    st("M12 8.5V12 M10.3 10.3 12 12 13.7 10.3"),
  ]),
  ...
  // --- Знаки розділів ----------------------------------------------------
  //  Пункт розділу мусить казати, ЩО це за розділ. Знаки нижче замінили
  //  позичені: торбу (`shop` — це магазин), стрічку (`feed` — це Простір),
  //  трикутник «відтворити» (`play`), іскри (`sparkles`) і глобус (`globe`).

  /** Оголошення — рупор: дошка оголошень це те саме, що крикнути на весь двір. */
  announce: icon([
    p("M3 10.5v3a1 1 0 0 0 1 1h2l9 4.5V6L6 10.5H4a1 1 0 0 0-1 1z"),
    p("M18.5 9.6a3.6 3.6 0 0 1 0 4.8"),
    p("M6.5 14.6V17a2 2 0 0 0 4 0v-1.3"),
  ]),

  /** Теми — палітра з фарбами: тема й палітра це одне й те саме слово. */
  palette: icon([
    p(
      "M12 21.5a9.5 9.5 0 1 1 9.5-9.5c0 2-1.5 3.2-3.3 3.2h-1.6a1.9 1.9 0 0 0-1.5 3 2 2 0 0 1-3.1 3.3z",
    ),
    c(8, 9.8, 1.1),
    c(11.6, 7.2, 1.1),
    c(15.6, 9.2, 1.1),
  ...
  /** Ігри — геймпад: хрестовина й кнопки. Це не «відтворити» (`play`). */
  game: icon([
    r(2, 8, 20, 9, 4.5),
    l(7.5, 10.5, 7.5, 14.5),
    l(5.5, 12.5, 9.5, 12.5),
    l(16, 11.6, 16, 11.6),
    l(18.4, 13.4, 18.4, 13.4),
  ]),

  /** Локації — позначка на карті: місце, а не «світ» (`globe`). */
  pin: icon([p("M20 10.4c0 5.4-8 11.6-8 11.6s-8-6.2-8-11.6a8 8 0 0 1 16 0z"), c(12, 10.2, 2.8)]),

  /** Контакти — картка людини з рядками: записник, а не конверт (`mail`). */
  contact: icon([
    r(3, 4.5, 18, 15, 2),
    c(9.2, 10.6, 2.2),
    p("M5.9 16.9a3.7 3.7 0 0 1 7.2 0"),
    l(14.6, 10, 18.2, 10),
  ]),

  /** Сторінки — аркуш із загнутим кутом: сторінка, яку людина створила. */
  page: icon([
    p("M13.5 2.5H6.5a2 2 0 0 0-2 2v15a2 2 0 0 0 2 2h11a2 2 0 0 0 2-2V8.5l-6-6z"),
    pl("13.5 2.5 13.5 8.5 19.5 8.5"),
  ]),

  /** Файли — тека: документи й архіви живуть у теці, а не на планшеті. */
  folder: icon([
    p("M3 7.5a2 2 0 0 1 2-2h4l2 2.5h8a2 2 0 0 1 2 2v9a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"),
    l(3, 10.5, 21, 10.5),
  ]),

  /** Тариф — корона: рівень плану, а не «іскри» (`sparkles`). */
  crown: icon([p("M2.5 18.5 3.6 7.6l4.6 3.3L12 4.2l3.8 6.7 4.6-3.3 1.1 10.9z")]),

  /** Питання — знак «?»: часті питання й усе, що питає. */
  question: icon([
    c(12, 12, 9),
    p("M9.5 9.7a2.6 2.6 0 0 1 5 .9c0 1.7-2.4 1.9-2.4 3.5"),
    l(12, 17.2, 12, 17.2),
  ]),
};
