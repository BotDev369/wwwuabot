 ...
 * те, що власник про людину знає: ім'я, `@username`, примітки (і хештеги, для
 * яких правила живуть у `@wwwuabot/shared/tags`). Це різні речі, і змішуються
 * вони лише в одному рядку таблиці.
 *
 * **Telegram-id тут немає, і це не упущення.** Власник не знає id людини, яку
 * заводить, — його бачить лише бот, коли людина приходить за лінком. Поле
 * «впиши id» дозволяло б будь-яке число, і контакт ставав би «приєднаним», а
 * лінк — «використаним» без жодного переходу.
 *
