 ...
 * те, що власник про людину знає: ім'я, примітки (і хештеги, для яких правила
 * живуть у `@wwwuabot/shared/tags`). Це різні речі, і змішуються вони лише в
 * одному рядку таблиці.
 *
 * **Telegram-id і `@username` тут не поля вводу, і це не упущення.** Власник не
 * знає ні id людини, ні її хендла — їх бачить лише бот, коли людина приходить
 * за лінком. Поле «впиши id» дозволяло б будь-яке число, і контакт ставав би
 * «приєднаним», а лінк — «використаним» без жодного переходу; поле «впиши
 * хендл» дало б те саме для імені. `sanitizeContactUsername` лишається тут, бо
 * ним бот нормалізує **те, що віддав Telegram**.
 *
 * Друга редагована копія хендла — це ще й друге джерело того самого факту:
 * картка показує `@handle` з колонки, тож записане руками число жило б поруч із
 * тим, що дописав бот.
 *
...
/**
 * `@username` із того, що дає Telegram (або отримав клієнт): без `@`, у
 * нижньому регістрі, лише `a-z0-9_`.
 *
 * Регістр знижуємо, бо Telegram-юзернейми регістронезалежні: `@Karas` і `@karas` —
 * та сама людина, і два різні рядки в базі змушували б шукати її двічі.
 * Порожній результат — `null`, а не `""`: «не знаю юзернейма» це відсутність
