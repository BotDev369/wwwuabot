...
describe("ціна в картці", () => {
  ...
  it("сире числове значення форматується з розділювачем та символом гривні", () => {
    expect(productPriceLabel("1500")).toBe("1\u00a0500 ₴");
    expect(productPriceLabel("25000.50")).toBe("25\u00a0001 ₴");
  });

  it("вже форматована ціна залишається без змін", () => {
    expect(productPriceLabel("320 ₴")).toBe("320 ₴");
  });
});
