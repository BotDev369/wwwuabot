...
/**
 * Ненадіслана чернетка — **документ зі своїм номером**, а не слот на пару людей.
 *
 * Це власні дані того, хто пише (як нотатка чи контакт), а не частина переписки:
 * співрозмовник про неї не знає, і в `messages` немає жодного її рядка.
 *
 * **Адресат тут необов'язковий.** Людина може почати лист, ще не вирішивши,
 * кому він: текст — це те, що вона написала, і втрачати його нема чого.
 *
 * **Номер, а не пара.** Чернеток може бути кілька (у тому числі тому самому
 * адресатові — кожна зі своїм текстом), тож ключ `owner + peer` зробив би з них
 * один слот, який друга збережена чернетка тихо затирає. Номером її правлять і
 * прибирають, номер же й відрізняє «та сама чернетка» від «ще одна».
 */
export interface MessageDraft {
  /** Номер чернетки — нею її правлять прибирають. */
  id: number;
  /** Кому адресована; `null` — адресата ще не обрали. */
  peerId: number | null;
  body: string;
  updatedAt: string;
}
...
/**
 * Те, що форма віддає на збереження.
 *
 * `id` — **наявна** чернетка (її правлять тим самим шляхом, що й створюють
 * нову), `null` — нова. `peerId: null` — лист без адресата; порожнє тіло разом
 * із ним означає «чернетки більше немає» (див. `saveDraft` у `api-dev`).
 */
 ...
 * не надіслано (`drafts` — усі чернетки людини, найсвіжіші згори).
 */
