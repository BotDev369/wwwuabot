...
export const commerceBlocks: BlockDefinition[] = [
    ...
    props: {
      plans: oa(
        "Тарифні плани",
        {
          name: s("Назва"),
          price: s("Ціна"),
          period: s("Період", { default: "/міс" }),
          features: sa("Можливості"),
          highlighted: b("Виділений"),
          ctaText: s("Текст кнопки", { default: "Обрати" }),
        },
        ["name", "price"],
      ),
  ...
  block({
    ...
    props: {
      text: s("Текст відгуку"),
      author: s("Автор"),
      role: s("Посада"),
      avatar: s("URL аватара", { format: "uri" }),
      rating: n("Рейтинг (0-5)"),
    },
  ...
  block({
    ...
    props: {
      items: oa("Фічі", { icon: s("Іконка"), title: s("Заголовок"), description: s("Опис") }, [
        "title",
      ]),
  ...
  block({
    ...
    compatibleZones: ["main", "sidebar"],
    props: {
      items: oa("Питання", { question: s("Питання"), answer: s("Відповідь") }, [
        "question",
        "answer",
      ]),
