 ...
 *   4. **Всі чотири джерела на місці** і кожне має охоронця `id`, тому
...
const SOURCES = [
  ...
  { table: "scenarios-admin", prefix: "sa:" },
...
describe("міграція контенту: схема", () => {
  it("`pages` оголошена, з унікальною адресою в межах групи й типу", () => {
    expect(TABLE_NAMES).toContain("pages");
    const indexes = TABLES.pages.indexes ?? [];
    expect(indexes.some((sql) => /UNIQUE INDEX.*\(kind, parent_id, slug\)/.test(sql))).toBe(true);
    // `codeword` унікальний лише коли заданий — інакше сторінки без бот-посилання
    // конфліктували б між собою на порожньому значенні.
    expect(indexes.some((sql) => /WHERE codeword <> ''/.test(sql))).toBe(true);
  });
...
describe("міграція контенту: повторний запуск", () => {
  it("усі чотири легасі-таблиці на місці", () => {
