...
| Сторінки з шаблону: два шаблони, переклад назви в адресу, правила, підпис бота | `packages/shared/src/pages/` — `templates.ts`, `address.ts`, `rules.ts`, `api.ts`, `bot.ts`; правила — `docs/PAGES.md` |
...
- **Тести:** Vitest, гейтять CI (червоний тест блокує деплой). Покриті: `security/`, `config/`, утиліти, `PageRenderer`, `content/`, роутинг і identity `api-dev`, реєстр таблиць D1, спільні кирпичики (`collection/`, `notes/`, `contacts/`, `pages/` — шаблони й виведений підпис бота), хром Telegram. Не покриті: сторінки оболонок і екрани бота.
...
- **Створити:** «+» у футері платформи — **сторінка** (`/create`), а не композер поверхні: слот, який щось відкриває, не має ні історії, ні «назад», ні посилання. Хаб — список власних екранів людини (Дати, Контакти, Локації, Нотатки, Оголошення, Повідомлення, Сторінки) за абеткою, і в кожного пункту **два входи, і вони різні**: «подивитись» веде на екран розділу (`replace` — хаб за спиною не лишається), а «створити» **не веде нікуди** — форма відкривається **поверх хабу** (нотатка, оголошення, сторінка з шаблону й лист — спільні кирпичики `@wwwuabot/ui/composer` / `@wwwuabot/ui/messages`; контакт заводять діалогом), тож її закриття (✕, збереження чи «назад» — у Telegram це найчастіше саме «назад») лишає людину в хабі. **Автоматичного переходу на іншу сторінку немає** — його робить окрема кнопка «подивитись». На екрані розділу форма навпаки — **запис історії** (`useCreateForm`: `…?new=1`, закриття вертає в розділ). Поверхню зводить `pages/create/CreateSheetHost.tsx`, список пунктів рендерить спільний `HubList` (`@wwwuabot/ui/hub`, рядки / плитки — той самий вибір, що в меню), склад і дії — `pages/create-hub.ts`. Правила — `docs/SURFACES.md`.
