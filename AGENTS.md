...
- **Простір:** `/space` — відкрита стрічка з вкладками («Користувачі» — перша й робоча; «Оголошення» й «Сторінки» названі, але ще без екранів, тож вкладка чесно каже, що там буде). Картку людини рендерить **один** спільний `PublicUserCard` — і в стрічці, і на сторінці людини (`/space/u/:id`); дані Telegram у публічне подання не потрапляють ніколи (тип `PublicProfile` цих полів не має). Фільтр видимості — у запиті до бази, тож закритий профіль відповідає тим самим 404, що й неіснуючий. Правила — `docs/SPACE.md`.
...
| Оголошення в Просторі (окрема таблиця, вкладка) і завантаження свого фото | `docs/SPACE.md`, «чого тут ще немає» |
