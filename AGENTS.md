...
- **Простір** — відкрита стрічка платформи (`/space`, слот футера): люди, які самі зробили свій профіль публічним, а далі — оголошення й сторінки. Адреса **власна**, а не `slug`: Простір — вибірка з даних (`users`), не рядок `scenarios`, і вигадати йому `slug` означало б завести друге правило «яка адреса відповідає цьому екрану». Правила видимості — `docs/SPACE.md`.
