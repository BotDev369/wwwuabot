...
- **Тема:** `/profile/theme` — **сторінки**, не модалка: хаб із п'ятьма розділами (Стиль, Готові теми, Мої схеми, Публічні схеми, Налаштувати), у кожного своя адреса. Між ними веде **друга смуга футера** (`SubBar` — спільний кирпичик, у розділі на 9% глибша за фон; модалка її не закриває). Схема = три кольори + шрифт (бренд у схему не входить) і живе в `theme_schemes`; «доступна публічно» виносить її в Простір. Правила — `docs/THEMES.md`.
