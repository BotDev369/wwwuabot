...
> (було 4 файли > 400; `sites.service.ts` поділено того ж дня, лишилось 3 із плану §3.5), і він тільки зменшується.
>
...
- **Масштаб (виміряно 12.09.2026):** **258 файлів `.ts`/`.tsx` у `src/` шести воркспейсів, 29 992 рядки** без тестів (було 327 і 35 198 — видалено 74 недосяжні файли; 12.09 додано 6 модулів замість `sites.service.ts`). Разом із тестами — 280 файлів, 32 004 рядки. Найбільші: `packages/shared/src/constants/site-templates.ts` (623), `web-admin-dev/src/pages/scenarios/ScenarioCardModal.tsx` (443), `packages/shared/src/components/icons.tsx` (423), `packages/shared/src/components/UserProfileCard.tsx` (417). Понад 200 рядків — 40 файлів логіки (з 255; ще 3 — дані, ліміт на них не діє), понад 400 — 6 (3 логіки, усі три в леджері боргу, і 3 даних).
