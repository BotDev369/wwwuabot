...
> **Версія:** 2.12 | **Останнє оновлення:** 13.09.2026
>
> **Зміна 2.12:** адреса сторінки перестала існувати двічі: було **дві колонки з тим самим
> рядком** (`web_slug` і `codeword`), через що «яка сторінка відповідає цьому URL» писалось
> чотири рази. Тепер сутність одна — `slug`: веб `/mydate/1980-03-03/today` і бот
> `?start=mydate_1980-03-03_today` будує `packages/shared/src/content/resolve.ts`; розділювач `_`
> диктує Telegram, тож сегмент не може містити `_` (два дефекти міграції — §7). Деталі — `docs/log/journal-2026-09-13-slug-entity.md`.
>
> **Зміна 2.11:** контент зведено в **одну таблицю** `pages`: рядок — це сторінка вебу (`blocks`)
> **і** її подання в боті (`bot`), `kind = 'collection'` — колишній сайт. Міграція —
> `scripts/migrate-content.sql` + `npm run migrate:content` (лише `INSERT`, ідемпотентно, зі звітом
> про пропущене; перевірено на SQLite). Деталі — `docs/log/journal-2026-09-13-pages-table.md`.
>
> **Зміна 2.10:** контент сторінок отримав спільну модель — `packages/shared/src/content/`
> (`ContentPage`, адаптери з трьох сховищ, вибір сторінки за URL). Той самий `PageConfig` лежав у
> **трьох** таблицях, а два справжні дефекти були невидимі: вкладку «Адмін» поза адмінкою не читав
> **ніхто**, а `title`/`photo_url` не вибирались із бази, хоч клієнт їх читав, — тому в блоках ці
> поля завжди були порожні. Цільова модель — `docs/CONTENT_MODEL.md`, деталі —
> `docs/log/journal-2026-09-13-content-model.md`.
>
> **Зміна 2.9:** схема D1 описана **один раз** — у реєстрі `packages/shared/src/database/tables.ts`
> (ім'я, власник, `CREATE TABLE`, індекси). Раніше DDL був розсипаний по чотирьох файлах,
> `scenarios` створювалась без `IF NOT EXISTS`, а `scenarios-admin` додавав собі колонки, ловлячи
> `no such column` — схема росла від того, що надіслав інтерфейс. `users` і `mydate_analysis` не
> створював **ніхто** (жили лише тому, що їх завели руками в дашборді). Стереже `npm run check:db`
> (§6), карта — `docs/DATA_MODEL.md`, деталі — `docs/log/journal-2026-09-13-d1-schema-registry.md`.
>
> **Зміна 2.8:** домен `services/sites` покрито тестами (68 тестів), і вони одразу знайшли
> справжній дефект: у `/api/sites/:slug/pages/:pid` власник перевірявся лише за сайтом, тому свій
> `slug` + чужий `pageId` давав запис у **чужу** сторінку (IDOR). Виправлено — сторінка мусить
> належати сайту з URL, і перевірка йде **до** підказок про існування об'єкта (§7 — правило про
> два сегменти URL). Окремо `dependabot.yml`: прибрано записи для неіснуючих тек і мінорні/патчові
> оновлення згруповано в один PR (11 окремих PR на тиждень з телефона не переглянути, а їхні гілки
> конфліктують у `package-lock.json`); мажорні й безпекові — окремо.
> Деталі — `docs/log/journal-2026-09-13-sites-tests.md` і `docs/log/journal-2026-09-13-dependabot-config.md`.
>
> **Зміна 2.7:** документацію приведено до того самого правила, що й код: один документ =
> одна тема. Моноліти поділено механічно, без переказу (жодне слово історії не втрачено):
> `CONSOLIDATION_LOG.md` 1 235 рядків → покажчик + 13 частин у `docs/log/`, `SITES_SPEC.md`
> 765 → покажчик + 8 частин у `docs/sites/`, аудит і дизайн-система — по дві частини.
> Нумерація § збережена, тож посилання з коду працюють без правок. З'явились
> `docs/README.md` (єдиний покажчик і таблиця «куди писати нове») і гейт `npm run check:docs`:
> розмір документа (400 — помилка, 200 — попередження), мертві посилання і «LOG §N» з коду,
> що ведуть у нікуди. Чому саме так — `docs/log/journal-2026-09-12-docs-refactor.md`.
>
> **Зміна 2.6:** планка в CI — `npm run check:quality` (ліміт рядків файлу, заборона нативних
> `alert`/`confirm`/`prompt`, голого `100vh` і емодзі в UI). Гейт знайшов і закрив 16 емодзі;
> `no-explicit-any` переведено з `warn` у **`error`** в усіх п'яти конфігах. Деталі —
> `docs/log/journal-2026-09-12-quality-gate.md`.
>
> **Зміни 2.1–2.5** (каркас оболонок зі спільних кирпичиків, гейт «клас без правила»,
> видалення 74 недосяжних файлів, поділ плану й журналу) — у `docs/CONSOLIDATION_LOG.md`
> і `git log`. Ранні правила не видалені: вони живуть у §3–§7 нижче, де їм і місце.
...
| Перенос контенту в `pages` (4 легасі-таблиці → одна) | `scripts/migrate-content.sql` + `npm run migrate:content` |
...
- Не створюй **другу таблицю під той самий контент**. Контент сторінки живе в одному рядку `pages`: `blocks` — сторінка вебу, `bot` — подання в боті, `kind = 'collection'` — група сторінок. Нова ознака контенту = колонка в `pages`, а не таблиця поруч; інакше з'явиться п'яте сховище одного `PageConfig`, а з ним і п'ята реалізація правила «яка сторінка для цього URL».
- Перенос даних між таблицями — окремим SQL (`scripts/migrate-content.sql`), який **тільки додає** (`INSERT`, ніколи `DELETE`/`DROP`/`UPDATE`), ідемпотентний (детермінований `id`) і **називає у звіті** все, що пропустив. Рядок, який не пройшов перевірку (чужа адреса чи чужий `codeword`), лишається в старій таблиці: вибрати за власника «правильний» — це тихо втратити чужий контент.
- Не покладайся на унікальність **імені індексу**: у SQLite імена індексів глобальні для бази, тому `CREATE UNIQUE INDEX IF NOT EXISTS idx_pages_slug` на другій таблиці — не помилка, а **порожня дія**, і таблиця лишається без унікальності. Стереже `tables.test.ts` (збіги імен) і раннер міграції (перевіряє і таблицю-власника індексу).
...
- Не парси `page_data` власним кодом і не пиши четверту копію правила «яка сторінка відповідає цьому URL» — бери `@wwwuabot/shared/content`. Фільтр видимості лишається у сховища (SQL або список у пам'яті): у редакторі чернетка мусить бути видимою, назовні — ні. Цільова модель і фази — `docs/CONTENT_MODEL.md`.
- Не тримай адресу сторінки в двох колонках (`codeword` і `slug`) і не пиши другу функцію «зробити з адреси посилання»: сутність одна — `slug`, подання будує `toWebPath` / `toBotPayload`. Алфавіт `?start=` задає Telegram (`A-Za-z0-9_-`, ≤ 64 символи), тому **сегмент адреси не може містити `_`** — це перевіряє `isValidSlug`, а довжину діплінка — `isDeepLinkable` (на побудові посилання, а не на переході: обрізаний параметр веде в нікуди й дізнатись про це нічим).
- Не лишай у міграції дитину без її групи й не мовчи про зайву колонку. Сторінка сайту їде тільки якщо доїхав її сайт (`AND EXISTS (… p.id = r.parent_id)`) — інакше в новій таблиці з'являється сирота, якої не бачить ні навігація, ні власник. А `ensureTables` уміє лише **додавати** колонки, тож таблиця, створена давнішою схемою, жила б із мертвою колонкою вічно: `npm run migrate:content` перевіряє це до вставки й дає команду ремонту.
- Не перевіряй право власника за одним сегментом URL. Якщо в шляху два незалежні ідентифікатори (`/api/sites/:slug/pages/:pid`), перевірка «сайт мій» **не** захищає сторінку: чужий `pid` під власним `slug` проходить її й пише в чужий об'єкт. Перевіряй зв'язок (`page.siteId === site.id`) і роби це **до** будь-яких підказок про існування об'єкта — код відповіді (`400` проти `404`) теж витік.

---

## 8. Статус проєкту

- **Масштаб:** числа (файли, рядки, тести, найбільші файли, леджер боргу) — **тільки** в `docs/CONSOLIDATION_PLAN.md` §0; тут їх немає, щоб не було двох правд. Структурно: шість воркспейсів, `sites.service.ts` більше не існує — замість нього шість модулів `api-dev/src/services/sites/*`, і всі вони тепер під тестами.
...
- **Схема D1:** усі таблиці — в одному реєстрі (`packages/shared/src/database/tables.ts`: дані; створення — `ensure-tables.ts`). Поза реєстром таблиць немає: `npm run check:db` (крок `D1 schema`) ловить `CREATE TABLE`, ім'я таблиці й `ALTER TABLE`, яких немає в оголошенні. Контент зводиться в одну таблицю `pages` — карта й міграція в `docs/DATA_MODEL.md`, фази в `docs/CONTENT_MODEL.md`.
