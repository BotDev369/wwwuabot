...
- **Створити:** «+» у футері платформи — **сторінка** (`/create`), а не композер поверхні: слот, який щось відкриває, не має ні історії, ні «назад», ні посилання. Хаб — список власних екранів людини (Дати, Контакти, Локації, Нотатки, Оголошення, Повідомлення, Сторінки) за абеткою, і в кожного пункту **два входи** — «подивитись» і «створити»; «створити» веде в **той самий** екран із наміром `?new=1` (`withCreateIntent`), бо друга форма в хабі розійшлася б із першою. Форма — **запис історії**, а не прапорець у стані екрана (`useCreateForm`): відкриття додає запис `…?new=1`, а під ним хаб кладе сам розділ, тож закриття форми (✕, збереження чи «назад» — у Telegram це найчастіше саме «назад») лишає людину в розділі; сам хаб після вибору замінюється обраним екраном (`replace`). Список пунктів рендерить спільний `HubList` (`@wwwuabot/ui/hub`, рядки / плитки — той самий вибір, що в меню), склад і адреси — `pages/create-hub.ts`. Правила — `docs/SURFACES.md`.
