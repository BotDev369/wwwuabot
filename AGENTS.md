...
| Композер | `.wb-composer*`, `.wb-modal--full`, `.wb-modal-overlay--tight` (рендерить спільний `ComposerModal`; відкриває «+» футера через `withPrimaryAction`; вкладки — дані в `tabs.ts`, заглушка чесно називає, що буде; мірки `--composer-cell/-gap/-pad` спільні для стовпчика вкладок і смуги вкладень, ліній усередині немає) |
