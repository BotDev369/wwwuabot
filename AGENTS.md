...
- Не додавай у розмітку клас, для якого немає правила. `class="wb-mt-3"` без `.wb-mt-3` не ламає ні збірку, ні тести — він просто нічого не робить, і помітити це можна лише очима. Перед уживанням перевір: `grep -n "\.wb-mt-3" packages/shared/src/styles/*.css`.
