...
>
> **Зміна 2.26:** лінії прибрано **скрізь** (правило 15): без рамок тепер картки, модалки, таблиці (рядок читається смугою, а не штрихом), меню й шапки, чипи, кнопки — і в спільному шарі, і **в брендах**; а плаваюча кнопка меню більше не лягає на композер (шари + `:has()`); у полів — нижня межа шрифту `max(16px, 1em)`, бо iOS сам збільшує сторінку на полі зі шрифтом < 16px (Android — ні, звідси різниця між ними), а висота повноекранної модалки береться від оверлея (`100%`), а не з `dvh`.
>
...
| Поля вводу | `.wb-input`, `.wb-select`, `.wb-textarea` + власні поля композера (`.wb-composer-input/-tags`) — **одне правило поверхні** на всі: колір `--field-bg` і м'яка тінь `--field-ring` (схема — `themes.css`). Межу поля малює колір, а не лінія: бренди більше не нав'язують `!important`-рамку, а додають лише радіус і мірки |
