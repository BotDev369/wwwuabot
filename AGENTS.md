...
> **Версія:** 2.14 | **Останнє оновлення:** 13.09.2026
>
> **Зміна 2.14:** тестову копію таблиці сценаріїв прибрано **повністю**: жодної таблиці
> `scenarios-admin`, маршрутів `/api/admin/scenarios/*`, вкладки «Адмін» у меню, типу `ScenarioTable`
> у клієнті чи гілки в моделі контенту. Її контент не показувався нікому поза адмінкою, тож
> переносити нічого — `pages` лишається єдиним сховищем. Деталі — `docs/log/journal-2026-09-13-drop-admin-table.md`.
>
> **Зміна 2.13:** «Поділитись» — третє подання тієї самої адреси: `https://t.me/<bot>?start=<slug>`
> (`buildShareLinks()`, `shared/content/link.ts`). Воно повертає **причину**, чому діплінка немає
> (`invalid_slug`, `no_bot_username`, `too_long`): Telegram обрізає задовгий параметр і приймає
> `@bot` **мовчки**. Вкладка «Поділитись» у картці рядка — третій бік `pages` поруч зі «Сторінка»
> і «Бот». Деталі — `docs/log/journal-2026-09-13-share-deeplink.md`.
>
> **Зміна 2.12:** адреса сторінки перестала існувати двічі (**дві колонки з тим самим рядком** —
> `web_slug` і `codeword`), через що «яка сторінка відповідає цьому URL» писалось чотири рази.
> Тепер сутність одна — `slug`; будує її `shared/content/resolve.ts` (розділювач `_` диктує
> Telegram — §7). Деталі — `docs/log/journal-2026-09-13-slug-entity.md`.
>
> **Зміна 2.11:** контент зведено в **одну таблицю** `pages`: рядок — це сторінка вебу (`blocks`)
> **і** її подання в боті (`bot`), `kind = 'collection'` — колишній сайт. Міграція (лише `INSERT`,
> ідемпотентно, зі звітом про пропущене) — `npm run migrate:content`; перевірено на SQLite.
> Деталі — `docs/log/journal-2026-09-13-pages-table.md`.
>
...
> **трьох** таблицях, а два дефекти були невидимі: вкладку «Адмін» не читав **ніхто**, а
> `title`/`photo_url` не вибирались із бази, тому в блоках вони завжди були порожні. Деталі —
...
> (ім'я, власник, `CREATE TABLE`, індекси): DDL був розсипаний по чотирьох файлах, а `users` і
> `mydate_analysis` не створював **ніхто**. Стереже `npm run check:db` (§6), карта й деталі —
> `docs/DATA_MODEL.md` і `docs/log/journal-2026-09-13-d1-schema-registry.md`.
>
...
> мінорні/патчові оновлення згруповано в один PR, мажорні й безпекові — окремо. Деталі —
> `docs/log/journal-2026-09-13-sites-tests.md`, `docs/log/journal-2026-09-13-dependabot-config.md`.
>
> **Зміна 2.7:** документацію приведено до того самого правила, що й код: один документ =
> одна тема. Моноліти поділено механічно: `CONSOLIDATION_LOG.md` → покажчик + 13 частин у
> `docs/sites/`; нумерація § збережена, тож посилання з коду працюють. З'явились `docs/README.md` і
> гейт `npm run check:docs`. Деталі —
...
> `100vh`, емодзі); `no-explicit-any` — з `warn` у **`error`**. Деталі — `docs/log/journal-2026-09-12-quality-gate.md`.
>
