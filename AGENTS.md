...
>
> **Це опис поточного стану, а не журнал.** Історії змін, дат «коли з'явилось» і виміряних чисел тут немає: історія живе в `git log`, а числа друкують самі гейти (`npm run check:css`, `check:docs`, `check:quality`). Число, вписане в документ, через тиждень стає неправдою — і його читають як факт.
...
- **Не заводи історії в документах.** Ніяких «було / стало», дат появи правил, журналів виконаного й хронологій: історія — це `git log`, а документ описує **тільки поточний стан**. Два документи про те саме — теж дефект: факт має одного власника.
...
**Відкрита робота** (усе інше закрито, і закритого в документах не тримаємо):
...
- **Жодних історій.** Не дописуй «що змінилось», не додавай дат появи правил, не заводь журналів виконаного: закрита робота — це коміт у `git log`. Документ описує те, як система влаштована **зараз**.
