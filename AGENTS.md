...
>
> **Зміна 2.24:** «+» футера відкриває **композер** — спільну модалку майже на весь екран (`@wwwuabot/ui/composer`): вкладки, типова — «Нотатка» з полем і вставкою, решта — чесні заглушки. Слот дії футера отримав `onSelect` (`withPrimaryAction`): «створити» — дія, а не адреса.
>
...
| Композер | `.wb-composer*`, `.wb-modal--full`, `.wb-modal-overlay--tight` (рендерить спільний `ComposerModal`; відкриває «+» футера через `withPrimaryAction`; вкладки — дані в `tabs.ts`, заглушка чесно називає, що буде) |
