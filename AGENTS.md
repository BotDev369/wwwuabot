...
- **Сторінка з шаблону** — сторінка, яку людина створила сама, змінюючи **лише текст**: два шаблони («Візитка», «Подія»), структуру задає шаблон, а `buildPageConfig` / `readPageValues` переводять значення в `page_data` і назад (`packages/shared/src/pages/`). Це **той самий** рядок `scenarios`, лише з власником (`owner_id`), прапорцем видимості (`is_public`, типово приватна) і ключем шаблону (`template_key`): другої таблиці під контент немає. Приватна сторінка недосяжна **і в боті**, бо бот — такий самий публічний вхід, як веб; правила — `docs/CONTENT_MODEL.md` і `docs/SPACE.md`.
- **Магазин** — сторінка, яку продавець зробив із шаблону «Магазин»: **той самий** рядок `scenarios` (`owner_id`, `is_public`, `slug` — його адреса, `id` — номер магазину, на який дивляться товари й замовлення), а не друга таблиця. Товар — окремий рядок (`shop_products`, адреса унікальна **в межах магазину**), замовлення — `shop_orders` зі знімком позицій; адреса товару — хвіст адреси магазину (`/<магазин>/p/<товар>`). Правила — `docs/SHOPS.md`.
- **Вид товару** — фізичний / цифровий / послуга: **закритий** перелік у коді (`PRODUCT_KINDS`), бо вид — це поведінка (доставка, файл), а не слово, за яким стоїть код. Свої слова магазин має там, де вони нічого не ламають — у статусах замовлення: ключі типові, підписи його. Правила — `@wwwuabot/shared/shop`.
- **Замовлення** — рядок `shop_orders` плюс **знімок** позицій (`shop_order_items` копіюють назву, ціну й вид на момент замовлення), а `status` — **ключ** (`new`/`done`/…), не текст. Оповіщення продавцю — розмова платформи (позначка `is_system`), а не друга система: замовлення й **є** зв'язком покупця з продавцем. Ціна — текст («договірна» теж ціна). Правила — `docs/SHOPS.md`.
...
- **Design System** — подвійна тема Apple/Material через `data-brand` на `<html>` і **три кольори користувача** (фон / основний / акцент) через `data-colors` + `data-colors-mode`. CSS-токени: `packages/shared/src/styles/`; палітру з трьох кольорів виводить `user-colors.css`, а сам вибір живе в `localStorage` (ключі `wwwuabot-colors` / `wwwuabot-font`). Світлої / темної як вибору немає — схему рахує фон. Шрифт — безкоштовний набір із кирилицею (`styles/fonts.ts`), він теж вибір людини і перекриває бренд інлайном.
- **Схема теми** — іменований набір: три кольори + шрифт (**без** бренду), у таблиці `theme_schemes`; перемикач «доступна публічно» виносить її в Простір. Поточний вибір лишається в пам'яті пристрою, бібліотека — на сервері. Правила — `docs/THEMES.md`.
...
- Мобільна навігація — виїзний drawer (`--sidebar-w-vw`, `--scrim`, `--duration-slow`, `--ease`): той самий патерн в обох оболонках, різні лише пункти меню. Ширина — **одна частка екрана** на всі сайдбари, і кожен сайдбар-поверхня має власну кнопку закриття (`docs/DESIGN_SYSTEM.md`, правило 24).
...
- Не тримай замовлення магазину ні в рядку користувача, ні в тексті повідомлення: замовлення — рядок `shop_orders` зі **знімком** позицій, інакше правка ціни заднім числом переписує історію, а сам факт замовлення не існує ніде, крім листування. Стереже `docs/SHOPS.md`.
...
- **Тести:** Vitest, гейтять CI (червоний тест блокує деплой). Покриті: `security/`, `config/`, утиліти, `PageRenderer`, `content/`, роутинг і identity `api-dev`, реєстр таблиць D1, спільні кирпичики (`collection/`, `notes/`, `contacts/`, `pages/` — шаблони, перегляд і виведений підпис бота), екрани сторінок платформи (`web-platform-dev/src/pages/user-pages/` — список шаблонів, перегляд шаблону цілком, редактор, крок у адресі), хром Telegram. Не покриті: решта сторінок оболонок і екрани бота.
...
- **Створити:** «+» у футері платформи — **сторінка** (`/create`), а не композер поверхні: слот, який щось відкриває, не має ні історії, ні «назад», ні посилання. Хаб — список власних екранів людини (Дати, Контакти, Локації, Нотатки, Оголошення, Повідомлення, Сторінки) за абеткою, і в кожного пункту **два входи, і вони різні**: «подивитись» веде на екран розділу (`replace` — хаб за спиною не лишається), а «створити» **не веде нікуди** — форма відкривається **поверх хабу** (нотатка, оголошення й лист — спільні кирпичики `@wwwuabot/ui/composer` / `@wwwuabot/ui/messages`; контакт заводять діалогом), тож її закриття (✕, збереження чи «назад» — у Telegram це найчастіше саме «назад») лишає людину в хабі. **Автоматичного переходу на іншу сторінку немає** — його робить окрема кнопка «подивитись». Виняток один — «Сторінки»: у пункту стоїть `createPath` (адреса екрана `/pages/new`), бо сторінку з шаблону поверхня не вміщує — спершу шаблон **показують цілком**, а текст правлять на ній самій (`docs/PAGES.md`). На екрані розділу форма навпаки — **запис історії** (`useCreateForm`: `…?new=1`, закриття вертає в розділ). Поверхню зводить `pages/create/CreateSheetHost.tsx`, список пунктів рендерить спільний `HubList` (`@wwwuabot/ui/hub`, рядки / плитки — той самий вибір, що в меню), склад і дії — `pages/create-hub.ts`. Правила — `docs/SURFACES.md`.
- **Профіль:** у платформи `/profile` — хаб **про людину**: **рядок** акаунта (два фото й два імені, `#` / `@`) і «Тема» (веде на свій розділ — `/profile/theme`; решту власних екранів забирає хаб «Створити»), а дані живуть на `/profile/account` — колишня картка акаунта, розділена навпіл: вкладка «Платформа» (**одна** картка «Дані на платформі»: фото, ім'я, «Про себе», роль, тариф, статус, а внизу — перемикач «Публічний профіль» із полями під ним) і вкладка «Телеграм» (**одна** картка: шапка — фото, повне ім'я й юзернейм із `@` — і нижче перелік полів у ста́лому нашому порядку; рядок поля — один рядок, підпис зліва, значення справа; преміум видно завжди, порожні поля не показуються; нашого дампу й даних сеансу там немає) — `@wwwuabot/shared` → `UserAccountRow`. У панелі свого профілю немає — вхід там за паролем, тож вона показує акаунт сесії й **картку чужого користувача** (`UserProfileCard` — тепер лише адмінська: роль, тариф, права, сирі поля). Правило «рядок `users` → профіль» живе в одному місці (`web-admin-dev/src/shared/api/user-profile-row.ts`).
- **Тема:** `/profile/theme` — **сторінки**, не модалка: три пункти (Стиль, Готові теми, Налаштувати), у кожного своя адреса, а між ними веде **друга смуга футера** (`SubBar` — спільний кирпичик, у розділі на 9% глибша за фон). Готові теми діляться **вкладками**: платформа, свої, з простору (`presets-tabs.ts`). У продукті сутність зветься **тема** — три кольори + шрифт (бренд у неї не входить), живе в `theme_schemes`, а «Доступна публічно» виносить її в Простір; «схема» — лише внутрішнє ім'я (`ThemeScheme`). Правила — `docs/THEMES.md`.
- **Простір:** `/space` — відкрита стрічка з розділами: «Оголошення» (перші), «Теми», «Ігри», «Сторінки» й «Користувачі» (останні) — робочі; розділи стоять у **лівій панелі** (`.wb-nav*` + `.wb-space-*`, `styles/space-nav.css`), а не смугою вкладок: вона відкривається розгорнутою (знак і підпис), коли розділ не названо адресою, а після вибору згортається до самих знаків (тумблер — у шапці сторінки, поруч із назвою, бо своєї шапки панель не має); картки в кожному розділі — **без заливки** (`styles/space.css`), розділює повітря, а шеврон означає перехід; у «Сторінках» контенту платформи немає навмисно, його знаходять за адресами. **Ігри — єдина вкладка без даних:** власні екрани, кожна гра має адресу (`/space/g/:key`), правила — чисті функції (пʼять ігор: хрестики-нулики, камінь-ножиці-папір, вгадай число, 2048, знайди пару), а де є суперник — він поки що бот (сервера в партії немає); партія йде **на весь екран** (`wb-game-screen`: без рядка заголовка й без футера, назад — стрілкою в партії); **власна палітра гри** (`--game-*` на екрані й сцені `data-game`, `styles/games.css`) — тимчасовий виняток зі спільних токенів, обмежений одним блоком на гру, відсутністю кольорів у розмітці й **світлими** палітрами (темний «зал» вийшов похмурим). Картку людини рендерить **один** спільний `PublicUserCard` — і в стрічці, і на сторінці людини (`/space/u/:id`); дані Telegram у публічне подання не потрапляють ніколи (тип `PublicProfile` цих полів не має). Фільтр видимості — у запиті до бази, тож закритий профіль відповідає тим самим 404, що й неіснуючий. **Оголошення — своя таблиця** `ads` (не `scenarios`: там `slug` — `NOT NULL UNIQUE`, тобто **опублікований контент із адресою**, а оголошення адреси не має), а форму дає той самий композер («+» у хабі «Створити» → «Оголошення», він же `/space?tab=ads&new=1`): вкладка «Оголошення» існує **лише там, де є обробник збереження**, — у панелі її немає; керування дошкою — **один ряд** спільної смуги колекції (пошук, види, вигляд, «+» акцентною клітинкою), а дії під своїм — за **«трьома крапками»** в шапці картки (спільна поверхня `MenuModal`), бо рядок підписів під кожним оголошенням займав місце в стрічці, яку читають. Правила — `docs/SPACE.md`.
