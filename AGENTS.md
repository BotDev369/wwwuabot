...
> **Версія:** 2.11 | **Останнє оновлення:** 13.09.2026
>
...
> `scripts/migrate-content.sql` + `npm run migrate:content` (схема з реєстру, лише `INSERT`,
> ідемпотентно, пропущене — у звіт; перевірено на SQLite). Два знайдені дефекти — у §7.
...
> (`ContentPage`, адаптери з трьох сховищ, `pickContentPage`). Огляд показав, що той самий
> `PageConfig` лежить у **трьох** таблицях (`scenarios`, `scenarios-admin`, `site_pages`), а
> правило «яка сторінка відповідає цьому URL» було написано тричі по-різному. Дорогою знайдено
> два справжні дефекти: вкладку «Адмін» поза адмінкою не читає **ніхто** (її контент не
> показується ні в боті, ні на платформі), а `title`/`photo_url` не вибирались із бази, хоч
> клієнт їх читав, — тому в блоках ці поля завжди були порожні. Додано правило в §7.
> Цільова модель і фази — `docs/CONTENT_MODEL.md`; деталі —
...
> сайтом, тому свій `slug` + чужий `pageId` давав запис у **чужу** сторінку (IDOR). Виправлено —
> сторінка мусить належати сайту з URL, і перевірка йде **до** будь-яких підказок про існування
> об'єкта; у §7 додано правило про два сегменти URL. Окремо впорядковано `dependabot.yml`:
> прибрано записи для неіснуючих тек (`/bot`, `/web`, `/api`, `/web-admin` — вони лишились від
> структури до монорепо й читались як покриття, якого немає) і мінорні/патчові оновлення
> згруповано в один PR — бо 11 окремих PR на тиждень нереально переглянути з телефона, а їхні
> гілки конфліктують у `package-lock.json`. Мажорні й оновлення безпеки лишаються окремо.
