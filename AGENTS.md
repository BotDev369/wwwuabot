...
> **Версія:** 2.13 | **Останнє оновлення:** 13.09.2026
>
> **Зміна 2.13:** «Поділитись» стало частиною моделі, а не кнопкою в інтерфейсі: третє подання
> тієї самої адреси — посилання `https://t.me/<bot>?start=<slug>` (`buildShareLinks()` у
> `packages/shared/src/content/link.ts`). Воно повертає **причину**, чому діплінка немає
> (`invalid_slug`, `no_bot_username`, `too_long`), бо Telegram обрізає задовгий параметр і приймає
> `@bot` **мовчки**. У картці рядка адмінки є вкладка «Поділитись» — разом зі «Сторінка» і «Бот»
> вона й робить рядок `pages` одним цілим (`docs/log/journal-2026-09-13-share-deeplink.md`).
>
...
> `?start=mydate_1980-03-03_today` будує `shared/content/resolve.ts`; розділювач `_` диктує
> Telegram, тож сегмент не може містити `_` (§7). Деталі — `docs/log/journal-2026-09-13-slug-entity.md`.
>
...
> **трьох** таблицях, а два дефекти були невидимі: вкладку «Адмін» поза адмінкою не читав **ніхто**,
> а `title`/`photo_url` не вибирались із бази, тому в блоках ці поля завжди були порожні. Деталі —
> `docs/log/journal-2026-09-13-content-model.md`; цільова модель — `docs/CONTENT_MODEL.md`.
>
...
> `scenarios-admin` додавав собі колонки, ловлячи `no such column`, а `users` і `mydate_analysis`
> не створював **ніхто** (жили лише тому, що їх завели руками в дашборді). Стереже
> `npm run check:db` (§6), карта — `docs/DATA_MODEL.md`, деталі — `docs/log/journal-2026-09-13-d1-schema-registry.md`.
>
> **Зміна 2.8:** домен `services/sites` покрито тестами (68), і вони одразу знайшли справжній
> дефект: у `/api/sites/:slug/pages/:pid` власник перевірявся лише за сайтом, тому свій `slug` +
> чужий `pageId` давав запис у **чужу** сторінку (IDOR). Виправлено: сторінка мусить належати сайту
> з URL, і перевірка йде **до** підказок про існування об'єкта (§7). Окремо `dependabot.yml`:
> прибрано записи для неіснуючих тек, мінорні/патчові оновлення згруповано в один PR, мажорні й
> безпекові — окремо. Деталі — `docs/log/journal-2026-09-13-sites-tests.md`,
> `docs/log/journal-2026-09-13-dependabot-config.md`.
>
> **Зміна 2.7:** документацію приведено до того самого правила, що й код: один документ =
> одна тема. Моноліти поділено механічно, без переказу: `CONSOLIDATION_LOG.md` → покажчик +
> 13 частин у `docs/log/`, `SITES_SPEC.md` → покажчик + 8 частин у `docs/sites/`. Нумерація §
> збережена, тож посилання з коду працюють без правок. З'явились `docs/README.md` і гейт
> `npm run check:docs` (розмір, мертві посилання, «LOG §N» з коду). Деталі —
> `docs/log/journal-2026-09-12-docs-refactor.md`.
>
> **Зміна 2.6:** планка в CI — `npm run check:quality` (ліміт рядків файлу, заборона нативних
> `alert`/`confirm`/`prompt`, голого `100vh` і емодзі в UI); `no-explicit-any` переведено з `warn`
> у **`error`** в усіх п'яти конфігах. Деталі — `docs/log/journal-2026-09-12-quality-gate.md`.
>
> **Зміни 2.1–2.5** (каркас оболонок, гейт «клас без правила», −74 файли, поділ плану й журналу) —
> `docs/CONSOLIDATION_LOG.md` і `git log`; ранні правила живуть у §3–§7.
