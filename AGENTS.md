...
> **Версія:** 2.31 | **Останнє оновлення:** 16.09.2026
>
...
- **CSS:** 12 файлів, 6 627 рядків (було 7 366 до «одного дизайну»); розбір по теках — `docs/CONSOLIDATION_PLAN.md` §0, правила й кирпичики — `docs/DESIGN_SYSTEM.md`.
...
- **Дизайн-система як кирпичики:** каркас обох оболонок — спільні `.wb-*` (`app-chrome.css`, §3). Перевірка `npm run check:css` (гейт CI) тримає дві межі: клас, який рендерить спільний код, стилізований у shared; клас у розмітці оболонки має правило. Поточно: 224 класи у спільному коді + 215 у оболонках, усі мають правила (470 класів у 12 CSS-файлах). Відомий борг — `scripts/css-baseline.mjs` (тільки зменшувати).
