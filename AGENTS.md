...
платформі ці правила не спрацьовували ніколи.
...
- **Дизайн-система як кирпичики:** каркас обох оболонок — спільні `.wb-*` (`app-chrome.css`, §3). Перевірка `npm run check:css` (гейт CI) тримає дві межі: клас, який рендерить спільний код, стилізований у shared; клас у розмітці оболонки має правило. Поточно: 218 класів у спільному коді + 215 у оболонках, усі мають правила (464 класів у 12 CSS-файлах). Відомий борг — `scripts/css-baseline.mjs` (тільки зменшувати).
