...
> **Версія:** 2.17 | **Останнє оновлення:** 13.09.2026
>
> **Зміна 2.17:** історію зведено в **один** архів `docs/HISTORY.md` (було 30 файлів: полотно
> + журнал на кожну роботу). Запис = рядок у §9, а не новий файл; повний текст видалених — у
> git (`git show <sha>:docs/log/…`). Два документи аудиту злито в `docs/CODE_QUALITY.md`.
>
> **Зміна 2.16:** таблицю `pages` і всю міграцію до неї **скасовано**: на дев-базі вона ніколи не
> існувала, читача не мала, а дані в ній були б ті самі, що вже в `scenarios` — п'яте сховище
> одного контенту. Сховище одне: **`scenarios`** (рядок = сторінка вебу + подання в боті);
> у реєстрі **4** таблиці — рівно як у базі. Деталі — `docs/HISTORY.md` §9.
>
> **Зміна 2.15:** домен «сайтів» прибрано повністю: таблиці `sites`, `site_pages`, `templates`
> (їх видалив власник) і **весь код навколо** — `services/sites/*`, 4 контролери з ~15 маршрутами
> (`/api/sites/*`, `/api/templates*`, `/api/catalog*`, адмін-модерація), редактор сайтів платформи,
> сторінки модерації й шаблонів адмінки, `SiteRenderer`, 9 документів специфікації. Деталі —
> `docs/HISTORY.md` §9.
>
> **Зміна 2.14:** тестову копію сценаріїв прибрано **повністю**: жодної таблиці `scenarios-admin`,
> маршрутів `/api/admin/scenarios/*`, вкладки «Адмін», типу `ScenarioTable` чи гілки в моделі
> контенту. Переносити нічого. Деталі — `docs/HISTORY.md` §9.
>
> **Зміна 2.13:** «Поділитись» — третє подання адреси (`buildShareLinks()`,
> `shared/content/link.ts`). Повертає **причину** (`invalid_slug`, `no_bot_username`, `too_long`):
> Telegram обрізає задовгий параметр і приймає `@bot` **мовчки**. Деталі — `docs/HISTORY.md` §9.
>
> **Зміна 2.12:** адреса перестала існувати двічі (**дві колонки з тим самим рядком** — `web_slug`
> і `codeword`), через що «яка сторінка відповідає цьому URL» писалось чотири рази. Сутність
> одна — `slug` (`shared/content/resolve.ts`). Деталі — `docs/HISTORY.md` §9.
>
> **Зміна 2.11:** контент зводили в **одну таблицю** — спершу як нову `pages`; 13.09.2026 цю
> спробу скасовано (див. 2.16), сховищем лишилась `scenarios`. Деталі — `docs/HISTORY.md` §9.
>
> **Зміна 2.10:** контент сторінок отримав спільну модель — `packages/shared/src/content/`
> (`ContentPage`, адаптер зі сховища, вибір сторінки за URL). Два дефекти були невидимі:
> вкладку «Адмін» не читав **ніхто**, а `title`/`photo_url` не вибирались із бази. Деталі —
> `docs/HISTORY.md` §9; модель — `docs/CONTENT_MODEL.md`.
>
...
- **Дизайн-система як кирпичики:** каркас обох оболонок — спільні `.wb-*` (`app-chrome.css`, §3). Перевірка `npm run check:css` (гейт CI) тримає дві межі: клас, який рендерить спільний код, стилізований у shared; клас у розмітці оболонки має правило. Поточно: 176 класів у спільному коді + 202 у оболонках, усі мають правила (415 класів у 12 CSS-файлах). Відомий борг — `scripts/css-baseline.mjs` (тільки зменшувати).
