...
> **Версія:** 2.30 | **Останнє оновлення:** 16.09.2026
>
...
| Меню | `.wb-sheet`, `.wb-sheet-head` (спільна повноекранна поверхня — композер і меню), `.wb-menu-body/-list`, `.wb-menu-item` (+ `--soon`), `.wb-menu-item-icon/-text/-label/-hint/-check`, `.wb-menu-ident*` (рендерить спільний `MenuModal`; відкриває «Профіль» футера через `withAction`; склад пунктів — `profile-menu.ts` оболонки, а `selected` малює галочку — це вибір, як у панелі теми) |
