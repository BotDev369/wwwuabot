...
- **Ім'я на платформі** (`users.platform_username`) — **не** Telegram `username`: це ім'я, яке користувач обирає собі сам на wwwuabot, і саме воно є його іменем у продукті (`packages/shared/src/user/platform-username.ts` — єдині правила для TWA, адмінки й бота). Telegram `username` лишається даними Telegram; `telegram_json` — усе, що Telegram віддав про людину, як є. Плутати їх не можна: друге ми не обираємо і воно може зникнути. І `@` належить **лише** Telegram-хендлу: ім'я на платформі пишеться без нього там, де текст читає Telegram — `@слово` там стає посиланням на чужий акаунт, і «@karas» веде людину до незнайомця. Правило одне на продукт: `peerPublicLabel(peer, "telegram")` (`packages/shared/src/messages/peer.ts`), стережуть тести обох модулів.
...
- **Профіль:** один екран `UserProfileCard` на обидві оболонки; свого профілю в панелі немає — вхід там за паролем, тож вона показує акаунт сесії. Правило «рядок `users` → профіль» живе в одному місці (`web-admin-dev/src/shared/api/user-profile-row.ts`).
