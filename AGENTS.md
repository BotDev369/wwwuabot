...
- **Профіль:** у платформи `/profile` — хаб зі **рядком** акаунта (два фото й два імені, `#` / `@`), а дані живуть на `/profile/account` — колишня картка акаунта, розділена навпіл: вкладка «Платформа» (фото, ім'я, роль, тариф, статус) і вкладка «Телеграм» (усе, що віддав Telegram, як є; порожнє місце — `...`, нашого дампу й даних сеансу там немає) — `@wwwuabot/shared` → `UserAccountRow`. У панелі свого профілю немає — вхід там за паролем, тож вона показує акаунт сесії й **картку чужого користувача** (`UserProfileCard` — тепер лише адмінська: роль, тариф, права, сирі поля). Правило «рядок `users` → профіль» живе в одному місці (`web-admin-dev/src/shared/api/user-profile-row.ts`).
