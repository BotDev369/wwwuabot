        ...
        <>
            <PageTopbar>
                <div className="topbar-left">
                    <button className="btn btn--secondary" onClick={() => navigate("/scenarios")} title="До списку">←</button>
                ...
                <div className="topbar-right">
                    ...
                    <button className="btn btn--secondary" onClick={() => navigate(`/scenarios/${encodeURIComponent(codeword)}/edit`)}>
                        Форма
                    </button>
                    <button className="btn btn--primary" onClick={() => navigate(`/editor?cw=${encodeURIComponent(codeword)}`)}>
                        Rich
                    </button>
                </div>
            </PageTopbar>
            ...
            <div className="scn-preview-body">
                {state === "loading" ? (
                    <div className="empty-state"><p className="empty-state-text">Завантаження сценарію…</p></div>
                ) : state === "error" ? (
                    <div className="empty-state"><p className="empty-state-text">Помилка: {errorMsg}</p></div>
                ) : state === "notfound" ? (
                    <div className="empty-state"><p className="empty-state-text">Сценарій «{codeword}» не знайдено в базі.</p></div>
                ) : (
                                        ...
                                        <div className="tg-buttons">
                                            {buttonRows.map((rowBtns, i) => (
                                                <div className="tg-btn-row" key={i}>
                                                    {rowBtns.map((b, j) => (
                                                        <span className="tg-btn" key={j}>{b.text}</span>
                                                    ))}
                                                </div>
                                            ))}
                                        </div>
