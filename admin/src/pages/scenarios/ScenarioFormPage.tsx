...
const FIELD_DEFS: FieldDef[] = [
    { key: "photo_url", label: "photo_url", kind: "input", hint: "https://… (порожньо = Cloudinary-банер)" },
    ...
    return (
        <>
            <PageTopbar>
                <div className="topbar-left">
                    <button
                        ...
                        onClick={() => navigate(isEdit ? `/scenarios/${encodeURIComponent(codeword!)}` : "/scenarios")}
                        title="Назад"
                    >
                        ←
                    </button>
                ...
                <div className="topbar-right">
                    ...
                    <button className="btn btn--primary" onClick={handleSave} disabled={saving || state === "loading"}>
            ...
            <div className="scn-form">
                {state === "loading" ? (
                    <div className="empty-state"><p className="empty-state-text">Завантаження…</p></div>
                ) : state === "error" ? (
                    ...
                    <div className="scn-form-inner">
                        ...
                        <div className="scn-form-grid">
                            {FIELD_DEFS.map((def) => (
                                <div key={def.key} className={`scn-field${def.kind === "textarea" ? " scn-field--full" : ""}`}>
                                    <label className="scn-field-label">{def.label}</label>
                                    {def.key === "buttons" ? (
                                        <ButtonsField value={form.buttons ?? ""} onChange={(v) => setField("buttons", v)} />
                                    ) : def.kind === "textarea" ? (
                                        <textarea
