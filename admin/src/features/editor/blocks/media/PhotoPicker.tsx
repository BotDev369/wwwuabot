    ...
    return (
        <div className="photo-picker">
            <div className="photo-tabs">
                <button type="button" className={`photo-tab${tab === "cloudinary" ? " photo-tab--active" : ""}`} onClick={() => setTab("cloudinary")}>
                    Cloudinary
                </button>
                <button type="button" className={`photo-tab${tab === "url" ? " photo-tab--active" : ""}`} onClick={() => setTab("url")}>
                    R2 / URL
                </button>
            </div>
            ...
            {tab === "cloudinary" ? (
                <div className="photo-fields">
                    ...
                    <div className="photo-row">
                        <label>Розмір</label>
                        <input className="block-input" type="number" value={width} onChange={(e) => setWidth(parseInt(e.target.value, 10) || 0)} />
                        <span>×</span>
                        <input className="block-input" type="number" value={height} onChange={(e) => setHeight(parseInt(e.target.value, 10) || 0)} />
                    </div>
                    <div className="photo-row">
                        <label>Колір фону</label>
                        <input type="color" value={`#${bgColor.replace("#", "")}`} onChange={(e) => setBgColor(e.target.value.replace("#", ""))} />
                        <label>Вирівнювання</label>
                        <select className="block-select" value={align} onChange={(e) => setAlign(e.target.value as "left" | "center" | "right")}>
                            <option value="left">Ліво</option>
                            <option value="center">Центр</option>
                            <option value="right">Право</option>
                        </select>
                    </div>
                    <button type="button" className="photo-show-btn" onClick={showCloudinary}>Показати / Оновити</button>
                </div>
            ) : (
                <div className="photo-fields">
                    <input
