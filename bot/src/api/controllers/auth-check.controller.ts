...
export async function handleAuthCheck(request: Request, env: Env): Promise<Response> {
  ...
  if (!userIdStr) {
    return new Response(JSON.stringify({ exists: false, error: "user_id required" }), {
      status: 400,
  ...
  if (isNaN(userId)) {
    return new Response(JSON.stringify({ exists: false, error: "invalid user_id" }), {
      status: 400,
      headers: { "Content-Type": "application/json" },
    });
  }
  ...
  try {
    ...
    return new Response(JSON.stringify({ exists: false, error: String(error) }), {
      status: 500,
      headers: { "Content-Type": "application/json" },
    });
  }
