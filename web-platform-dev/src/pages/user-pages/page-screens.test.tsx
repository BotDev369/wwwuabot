...
describe("перегляд шаблону", () => {
  ...
  it("магазин показує сітку товарів, а не самі текстові картки", () => {
    // Товари лежать у своїй таблиці, і в перегляді шаблону їх ще немає — але
    // шаблон магазину без сітки виглядає парою абзаців, тобто рівно тим, чим
    // він не є (docs/SHOPS.md §3). Тому в перегляді блоку дістає приклад.
    const html = preview("shop");
    expect(html).toContain("shop-catalog-grid");
    expect(html.match(/class="shop-card"/g)?.length ?? 0).toBeGreaterThan(1);
    // І чесно називає себе прикладом: товарів у магазині ще немає.
    expect(html).toContain("shop-note");
  });

  it("сітка товарів стоїть **перед** умовами, а не під ними", () => {
    // У магазині спершу дивляться на товар: «Про магазин» і «Доставка» — те,
    // що читають після нього, а не перед.
    const html = preview("shop");
    expect(html.indexOf("Товари")).toBeLessThan(html.indexOf("Доставка й оплата"));
  });
});
