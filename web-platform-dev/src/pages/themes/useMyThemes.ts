/**
 * Власні теми — разом із закритими, і з діями над ними.
 *
 * **Окремо від спільної бібліотеки.** Власні читаються з-під підписаного
 * `initData` й містять закриті теми; спільна — публічна. Один запит на обидва
 * мусив би або відкрити закриті теми всім, або сховати від людини її ж
 * бібліотеку — тож це два хуки, і кожен тягне рівно те, що йому потрібно.
 *
 * **Після запису список перечитується, але не порожніє.** Тема — це річ, яку
    ...
    themesApi
      ...
      .catch((e: unknown) => {
        ...
        setState({
          items: [],
          loading: false,
          error: e instanceof Error ? e.message : "Не вдалося завантажити теми",
        });
      });
