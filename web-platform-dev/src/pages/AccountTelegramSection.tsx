 ...
 * фото, повне ім'я (`first_name` + `last_name`), `@юзернейм` і кожне поле, яке
 * прийшло в payload — зокрема те, якого ми ще не знаємо. Людина мусить упізнати
 * себе цілком, а не половину.
 *
 * **Порожньо — `...`, а не пропуск.** Юзернейма може не бути, і тоді другий
 * рядок шапки стоїть на своєму місці з `...`: рядок, що зникає, читався б як
 * «поламалось», а не як «його справді немає». У переліку полів порожніх рядків
 * немає взагалі — там вони не несуть нічого (`telegram-fields.ts`).
 *
