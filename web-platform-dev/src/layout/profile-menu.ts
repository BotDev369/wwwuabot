...
/**
 * Чи поверхня показує смугу внизу — перемикач вигляду й «Закрити».
 *
 * Лише список розділів: смуга потрібна там, де є **що** перемикати (картки) і
 * де вихід мусить бути під пальцем. Панель теми — форма: у неї свій вихід
 * у шапці й своя кнопка збереження, тож смуга з «Закрити» стояла б у ній без
 * роботи — і саме так вона виглядала зайвою (виходило два виходи на один
 * екран, один із них посеред екрана). */
export function hasBottomBar(view: ProfileMenuView): boolean {
  return view === "list";
}
