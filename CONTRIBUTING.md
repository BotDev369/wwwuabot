...
| Документація | `npm run check:docs` | бюджет розміру (рядки й вага) з правами на більше; мертві посилання й шляхи; свіжість `docs/API.md`; `AGENTS.md §N` |
...
3. **Кристалева ясність.** Файл > 200 рядків = червоний прапорець, > 400 = критично. Компонент =
   рендеринг. Хук = логіка. Хелпер = чисті функції. У документа свій бюджет (ще й за вагою) і
   право на більше — тільки з причиною, див. `docs/README.md`.
...
| **`docs/README.md`** | **покажчик усієї документації і бюджет розміру — почни звідси** |
| `docs/RECIPES.md` | Покрокові рецепти типових задач |

Решта документів — за покажчиком. Факт має **одного власника**: якщо правило чи число потрібне в
двох місцях, його тримає один документ, а інші посилаються. Реєстри (класи, ендпоїнти, таблиці)
руками не пишуться: їх друкують гейти або генерують (`npm run doc:api`).

Правило, яке додається в документацію, коштує **один рядок** там, де воно потрібне: правило — у
`AGENTS.md` (з причиною одним реченням), рецепт — у `docs/RECIPES.md`, нова тема — окремий
маленький `.md` за покажчиком. Файл, який переріс бюджет, падає в `check:docs` замість того, щоб
зростати непомітно.
