...
14. **Історію не пиши в документ: документ описує поточний стан.** Закрита робота — це коміт у
    `git log`, а не абзац «було / стало»; нове правило — у `AGENTS.md`, нова тема — маленький
    документ у `docs/` і рядок у покажчику.
...
Решта документів — за покажчиком. Факт має **одного власника**: якщо число чи правило потрібне в
двох місцях, його тримає один документ, а інші посилаються. Журналів виконаного й `CHANGELOG.md`
немає — історія в `git log`.
